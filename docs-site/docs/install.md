...
### 本番運用で確認しておきたい追加の環境変数

以下は開発用の既定値のままでも動作しますが、本番運用では内容を理解したうえで設定を検討してください。

- `ALLOWED_HOSTS`：アクセスを許可するホスト名・IPアドレスをカンマ区切りで指定します。**未設定の場合はソースコードにあらかじめ組み込まれた既定のホストにフォールバックします。** 開発環境と異なるホスト名・IPで公開する場合は、必ず自分の環境向けに設定してください。
- `CSRF_TRUSTED_ORIGINS`：nginx 等で SSL 終端して Django へプロキシする構成で、POST リクエストを受け付けるオリジンをスキーム付き・カンマ区切りで指定します（例：`https://your-domain.example.com`）。こちらも**未設定の場合はソースコードにあらかじめ組み込まれた既定のオリジンにフォールバックします。** `ALLOWED_HOSTS` と同様、公開ホスト名が既定と異なる場合は設定が必須です。
- `LOG_LEVEL`：サーバー側のログ詳細度です。既定は `INFO`。トラブル調査時に一時的に `DEBUG` へ変更すると、Django の内部ログが詳しく出力されます（`DEBUG=True` の画面表示モードとは別物で、本番でも安全に使えます）。運用中の使い分けは [メンテナンスコマンド](admin/maintenance.md) も参照してください。

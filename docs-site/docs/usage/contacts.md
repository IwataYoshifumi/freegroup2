...
コンタクト一覧の「インポート」から、CSVファイルをアップロードして複数のコンタクトを一括登録できます。アップロード後、取り込み件数や内容のプレビュー画面が表示されるので、内容を確認してから確定します。列見出しの対応や取り込み時の正規化ルールなど詳細な仕様は、開発元にお問い合わせください。

## CSVエクスポート

コンタクト単体を一覧からそのままCSVエクスポートする機能は、現時点ではありません。エクスポートは、メール配信のメーリングリストのメンバーを対象とした機能として提供されています。メーリングリストの詳細画面から、出力する項目を選んでCSV形式でダウンロードできます。vCard形式でのエクスポートは現時点で対応していません。

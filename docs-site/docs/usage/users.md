...
[案件管理](deals.md)・[活動記録](activities.md)・[会社管理](companies.md)には、Role に対応した権限グループが用意されています。

| Role | 案件・活動記録での扱い | 会社での扱い |
| --- | --- | --- |
| `admin` | すべての案件・活動記録を閲覧・編集できる | すべての会社を編集・重複統合できる |
| `sales` | 自分が担当（オーナー／実施者）している分、または関係者として登録されている分のみ閲覧・編集できる | 権限を持てば全会社を編集可（会社には個人単位の絞り込みはない） |
| `viewer` | 自分が関係者として登録されている案件・活動記録のみ閲覧できる。登録されていないものは一覧にも表示されない | 権限を持てば閲覧のみ可能 |

案件・活動記録は、上記の Role による大枠の権限に加えて、レコードごとの個別チェックが働きます。

- **案件・活動記録**：担当者（オーナー・実施者）本人であるか、その案件・活動記録に「関係者」として登録されているかで閲覧・編集の可否が決まります。関係者として編集を許可するかどうかは、関係者登録の際に個別に設定します。ただし、アーカイブや担当引き継ぎは、関係者に編集権限があっても行えず、担当者（オーナー）本人のみが行えます。
- **会社**：案件・活動記録のような個別レコード単位のチェックはありません。会社の編集・重複統合の権限を持つユーザーは、すべての会社に対して操作できます。

### メール配信（mailings）の権限

[メーリングリスト管理](mailing-lists.md)・[キャンペーン管理](campaigns.md)には、以下の6つの権限グループが用意されています。

| グループ | 対応する画面 | できること |
| --- | --- | --- |
| `campaign_admin` | キャンペーン | 全キャンペーンの閲覧・編集・送信・エクスポートに加え、メール配信のシステム設定変更も可能 |
| `campaign_editor` | キャンペーン | キャンペーンの作成・編集・送信・エクスポートが可能（自分が作成した分のみ） |
| `campaign_viewer` | キャンペーン | 自分が作成したキャンペーンの閲覧のみ可能 |
| `tag_admin` | タグ | タグ・カテゴリの作成・編集・削除、人物への付与すべてが可能 |
| `tag_editor` | タグ | タグの編集・人物への付与が可能 |
| `tag_viewer` | タグ | タグの閲覧のみ可能 |

**キャンペーンだけが「自分が作成したものか」で範囲が決まるオーナー制**になっています。`campaign_admin`のように「全キャンペーン閲覧」権限を別途持っていない限り、`campaign_editor`・`campaign_viewer`は自分が作成したキャンペーンしか見えません。

一方、**メーリングリスト・メールテンプレート・ブロックリストにはオーナー制がありません。** 該当の権限（閲覧・編集など）さえ持っていれば、誰が作成したものでも操作できます。

...
- **`仕様書_v1_6_メール配信_クリックトラッキング_rev19.md` §14（v1.6 認可設計：CRUD ベース Permission ＋起点モデル基準グループ名。本 rev18 で追従反映）**
...
| Group 名 | 含まれる Permission | 含まれる Role | 仕様書記載 |
|---|---|---|---|
| `campaign_admin` | `view_campaign`, `add_campaign`, `change_campaign`, `delete_campaign`, `send_campaign`, `view_all_campaigns`, `export_report`, `manage_template`, `manage_suppressed_email`, MailingList 全（`view/add/change/delete_mailinglist`）, `change_mailingconfig` | `admin` | ★1（rev19 §14.2.1） |
...
| `campaign_viewer` | `view_campaign`, `view_mailinglist` | `viewer` | ★1（rev19 §14.2.1） |

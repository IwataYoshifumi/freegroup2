...
バージョン v1.6.0
FreeGroup2 グループウェア
2026年5月作成 / OCR・OpenCV 章を v1.6.0 系 3 本に分離し本編仕様書として再編
...
| 2 | 旧 §15.5（正規化ルール）を §11.9（Contact 正規化基盤）として第 11 章末尾に移植。v1.6.0 統合版 第 6 部（3 経路共有の正規化基盤・AJAX 経路の正規化通し・compose_full_address・正規化 5 カテゴリ・org_domain_name 汎用ドメイン無視）を統合 |
...
| personal_phone / personal_fax | JSONField(default=list) | 個人直通電話 / 個人 FAX（**v1.6.0 で phone / fax からリネーム＋型変更**、E.164 配列） |
| org_phone / org_fax | JSONField(default=list) | 会社代表・部署電話 / FAX（v1.6.0 新規、E.164 配列） |
...
- 旧 第 5 章 OCR 結果 JSON 仕様 → v1.6.0 統合版 第 1 部・JSON 構造対応表
- 旧 第 6 章 画像処理仕様 → v1.6.0 統合版 第 4 部（OpenCV パイプライン）
- 旧 第 7 章 OCR バックエンド仕様 → v1.6.0 統合版 第 2 部・OCR プロンプト
- 旧 §15.1〜§15.3, §15.6 OCR パイプライン → v1.6.0 統合版 第 3 部・第 4 部
...
- 継承元：forms.ModelForm
- Meta.fields：Contact のユーザー入力対象フィールド（v1.6.0 リネーム後）。full_name / last_name / first_name / name_order / other_name_parts / salutation_name / display_name / phonetic_name / alias_name / organization / department / title / branch / email / mobile_phone / personal_phone / personal_fax / org_phone / org_fax / postal_code / region / city / rest_of_address / country / website / qualification / catchphrase / SNS各種 / notes / lang / language_composition / legal_entity_type / legal_entity_type_position など。導出フィールド（address / org_core_name / org_domain_name / legal_entity_type_code）と Contact 非保持（original_script / ai_analysis_notes）は含めない（§11.9.5・別表 A.5 参照）。salutation_name は必須バリデーション対象
...
**【v1.6.0 共通】salutation_name 必須バリデーション：** ContactCreateForm / ContactUpdateForm / ContactUpdateActiveForm は salutation_name を必須とする（空文字・空白のみは ValidationError）。DB は NULL 許容のまま、Form 側で必須を担保する。AJAX 経路（ContactAjaxUpdateFieldView）も salutation_name 空文字送信は 400 エラー（§11.9.3）。各 Form の clean で salutation_name の非空チェックを行う。
...
- `normalize_phone_value(raw)`：電話番号 1 件の正規化（配列フィールド personal_phone / personal_fax / org_phone / org_fax には配列要素ごとに適用）
...
| 経路 | 呼び出し元 |
|---|---|
| OCR 経路 | json_parser が OCR 出力読み取り時点で各フィールド値を純関数に通す（v1.6.0 統合版 §3.3.1 参照） |
...
※ full_name は経路依存。OCR 経路ではカテゴリD（original_script からのコピー、v1.6.0 統合版 §3.3.1 参照）、手動入力経路では UI 補助（last_name / first_name / other_name_parts / name_order から UI スクリプト補助組み立て＋手入力可）。下表ではカテゴリD と UI 補助の両方に full_name を記載しているが、経路によって扱いが分かれることを示す。
...
| C：二重検算 | 別ソース照合→不一致でconfidence low | legal_entity_type_code/電話系5/country/name ブロック整合性チェック（v1.6.0 統合版 §2.4.1） |
...
phonetic_name はカテゴリA（軽い整え）。normalization は体裁整えのみ（全角空白→半角・連続空白1つ・前後空白除去）。字数比較・構造照合はしない（カタカナと漢字で字数が一致しないため検算不能）。name ブロック整合性チェック（v1.6.0 統合版 §2.4.1）の対象外。
...
- **mobile_phone / personal_phone / personal_fax / org_phone / org_fax**：数字とハイフンのみ抽出 / ハイフン除去 / 全角数字→半角 / 国番号正規化 / 漢数字→半角（配列フィールドは各要素に適用）
- **email**：全体小文字化 / 前後空白除去
...
値：['full_name', 'organization', 'department', 'title', 'branch', 'email', 'personal_phone', 'mobile_phone', 'address']（9 フィールド。v1.6.0 で company→organization / phone→personal_phone / mobile→mobile_phone を機械的リネーム反映。フィールド数・スコア・ランク閾値・判定ロジック〈confidence=="high" のみ加算〉は不変。org_phone / org_fax は含めない＝会社代表電話は同一会社の複数人で同値のため重複検出に使うと別人が誤マージ）
...
| メール | email | 既存 |
| 個人直通電話 | personal_phone | **リネーム＋型変更（旧 phone、CharField → JSONField(default=list)）** |
| 個人携帯 | mobile_phone | **リネーム（旧 mobile）** |
| 個人 FAX | personal_fax | **リネーム＋型変更（旧 fax、CharField → JSONField(default=list)）** |
| ウェブサイト | website | 既存 |
| 資格 | qualification | 既存 |
| キャッチフレーズ | catchphrase | 既存 |
| Twitter | twitter | 既存 |
| Instagram | instagram | 既存 |
| GitHub | github | 既存 |
| LinkedIn | linkedin | 既存 |
| Facebook | facebook | 既存 |
...
| 会社代表・部署電話 | org_phone | **新規**（JSONField(default=list)：E.164 配列） |
| 会社・部署 FAX | org_fax | **新規**（JSONField(default=list)：E.164 配列） |

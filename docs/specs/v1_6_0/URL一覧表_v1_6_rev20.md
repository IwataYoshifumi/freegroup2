...
- **`仕様書_v1_6_メール配信_クリックトラッキング_ドラフト_rev12_3.md` §5.1 / §6.2（v1.6 メール配信・クリックトラッキング・タグ・リスト URL・画面設計）**
- **`仕様書_v1_6_メール配信_クリックトラッキング_rev21.md` §14（v1.6 認可設計：CRUD ベース Permission ＋起点モデル基準グループ名、view_suppressedemail を全ロール閲覧に割り当て、change_mailingconfig を Django 標準（自動生成）に是正。本 rev20 で追従反映）**
...
- **ロール**（v1.5.0 で確定、v1.6 で `campaign_admin` / `campaign_editor` / `campaign_viewer` / `tag_admin` / `tag_editor` / `tag_viewer` Group を追加）：
  - `admin`＝管理者（v1.6 で `campaign_admin` + `tag_admin` を default_groups に追加、rev19 §14.3）
  - `sales`＝営業（v1.6 で `campaign_editor` + `tag_editor` を default_groups に追加）
  - `viewer`＝閲覧者（v1.6 で `campaign_viewer` + `tag_viewer` を default_groups に追加）
...
| No. | ルート | メソッド | View 名 | URL name | ログイン | Permission | ロール | 備考 |
|---|---|---|---|---|---|---|---|---|
| 44 | `/mailings/templates/<uuid:pk>/update/` | GET / POST | EmailTemplateUpdateView | `mailings:template_update` | 必須 ★1 | `mailings.manage_template` ★1 | admin / sales ★1 | v1.6 新規（rev17 で改訂）。Campaign 詳細画面から「件名・本文を編集」ボタンで呼ばれる。本文はプレーンテキスト入力（rev9 確定、HTML タグは書かない、記法 3 種のみ）、`EmailContext.prepare` が HTML 化（rev12.3 §7.4 / §1.1）。編集画面にプレビューボタンを置き、§5.1 No.42 プレビュー View を Ajax で呼ぶ（rev17 §7.7.1 拡張） |
...
| 47 | `/mailings/campaigns/create/` | GET / POST | CampaignCreateView | `mailings:campaign_create` | 必須 ★1 | `mailings.add_campaign` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.7）。テンプレート選択、宛先＝リスト選択のみ（rev12 で `tag_condition` 削除・`mailing_list` 必須 FK 化）、予約配信日時（必須・未来日時のみ） |
...
| 49 | `/mailings/campaigns/<uuid:pk>/update/` | GET / POST | CampaignUpdateView | `mailings:campaign_update` | 必須 ★1 | `mailings.change_campaign` + 所有者判定 ★1 | 本人 / admin ★1 | **v1.6 新規**（rev12.3 §5.1 No.9）。draft 状態のみ可 |
| 50 | （★2 候補、コード君判断） | POST | 予約登録用 View（具体名はコード君判断） | （★2、`mailings:campaign_schedule` 等が自然だがコード君判断） | 必須 ★2 | `mailings.change_campaign` + 所有者判定 ★2 | 本人 / admin ★2 | **【rev9 で即時配信実行エンドポイント `/mailings/campaigns/<uuid:pk>/execute/` は廃止】 予約登録用 URL・View 名は仕様書では確定せず、コード君の実装時判断**（rev12.3 §5.1 No.10、rev8 B 群・C 群の流儀踏襲）。★2 候補：URL を本表で確定しない |
...
| No. | ルート | メソッド | View 名 | URL name | ログイン | Permission | ロール | 備考 |
|---|---|---|---|---|---|---|---|---|
| 57 | `/mailings/suppressed/` | GET | SuppressedEmailListView | `mailings:suppressed_list` | 必須 ★1 | `mailings.view_suppressedemail` ★1 | 全員 ★1 | **v1.6 新規**（rev12.3 §5.1 No.17）。SuppressedEmail 一覧 |
| 58 | `/mailings/suppressed/<uuid:pk>/` | GET / POST | SuppressedEmailDetailView | `mailings:suppressed_detail` | 必須 ★1 | GET=`mailings.view_suppressedemail` / POST=`mailings.manage_suppressed_email` ★1 | GET=全員 / POST=admin ★1 | **v1.6 新規**（rev12.3 §5.1 No.18）。`cancelled_at` の更新（解除） |
...
| No. | ルート | メソッド | View 名 | URL name | ログイン | Permission | ロール | 備考 |
|---|---|---|---|---|---|---|---|---|
| 60 | `/settings/system/` | GET / POST | SystemSettingsView | `mailings:settings_system` | 必須 ★1 | `mailings.change_mailingconfig` ★1 | admin のみ ★1 | **v1.6 新規**（rev12.3 §5.1 No.38）。Settings モデル編集（シングルトン、特電法対応の会社情報・差出メール・DKIM 設定等、rev12.3 §4.13）。Permission は rev19 §14.1.2 で `mailings.change_mailingconfig`（admin 専用）に確定（旧★2 仮称 `mailings.manage_settings` から確定済みに更新）。Django 標準 change_mailingconfig を流用（Meta に手書きしない） |
...
| 64 | `/tags/create/` | GET / POST | TagCreateView | `tags:tag_create` | 必須 ★1 | `tags.add_tag` ★1 | admin / sales（tag_admin / tag_editor） ★1 | **v1.6 新規**（rev12.3 §5.1 No.21）。タグ作成時は **`category` 必須 FK**（rev12 で `group_id` 削除→TagCategory FK 必須化、rev12.3 §4.9A）。TagCategory が 0 件だと Tag を作成できない（rev12.3 §4.9 初期データ方針：空スタート・運用者が手動作成） |
| 65 | `/tags/<uuid:pk>/` | GET | TagDetailView | `tags:tag_detail` | 必須 ★1 | `tags.view_tag` ★1 | 全員 ★1 | **v1.6 新規**（rev12.3 §5.1 No.22） |
| 66 | `/tags/<uuid:pk>/update/` | GET / POST | TagUpdateView | `tags:tag_update` | 必須 ★1 | `tags.change_tag` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.23） |
...
| No. | ルート | メソッド | View 名 | URL name | ログイン | Permission | ロール | 備考 |
|---|---|---|---|---|---|---|---|---|
| 70 | `/mailing-lists/` | GET | MailingListListView | `mailings:list_list` | 必須 ★1 | `mailings.view_mailinglist` ★1 | 全員 ★1 | **v1.6 新規**（rev12.3 §5.1 No.27）。リスト一覧。rev12 でリスト凍結方式確定（rev12.3 §11.3 / §4.11）。Permission は rev12.3 §14 で `mailings:list_*` 系の codename が未明示、★2 候補 |
| 71 | `/mailing-lists/create/` | GET / POST | MailingListCreateView | `mailings:list_create` | 必須 ★1 | `mailings.add_mailinglist` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.28）。【**【rev12 で全面改訂】 リスト作成画面の本体は別 URL（No.81 ★2 候補、§6.2.4 リスト作成画面）**】 本 No.28 は最小のリスト作成エンドポイント、画面の具体はコード君判断 |
| 72 | `/mailing-lists/<uuid:pk>/` | GET | MailingListDetailView | `mailings:list_detail` | 必須 ★1 | `mailings.view_mailinglist` ★1 | 全員 ★1 | **v1.6 新規**（rev12.3 §5.1 No.29）。リスト詳細（凍結メンバー表示）。**`extraction_snapshot` を使った再抽出機能は §19 論点 15 確定待ち・Phase 1 スコープ外**（rev12.3 §11.4.3.1 段階線引き、Phase 1 では `extraction_snapshot` はフィールド定義のみ） |
| 73 | `/mailing-lists/<uuid:pk>/update/` | GET / POST | MailingListUpdateView | `mailings:list_update` | 必須 ★1 | `mailings.change_mailinglist` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.30）。リスト名・説明等の編集 |
| 74 | `/mailing-lists/<uuid:pk>/delete/` | POST | MailingListDeleteView | `mailings:list_delete` | 必須 ★1 | `mailings.delete_mailinglist` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.31）。論理削除（リストは論理削除運用、`Campaign.mailing_list` PROTECT のため物理削除はブロックされる、rev12.3 §4.11 / §11.3.4） |
| 75 | `/mailing-lists/<uuid:pk>/add-member/` | POST | MailingListAddMemberView | `mailings:list_add_member` | 必須 ★1 | `mailings.change_mailinglist` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.32）。AJAX。**【マージ時は MailingListMember は付け替えない＝凍結スナップショット、rev12 改訂5 で §601 行矛盾是正済み、rev12.3 §4.12 / §9.4.1 / §11.7.2】** |
| 76 | `/mailing-lists/<uuid:pk>/remove-member/` | POST | MailingListRemoveMemberView | `mailings:list_remove_member` | 必須 ★1 | `mailings.change_mailinglist` ★1 | admin / sales ★1 | **v1.6 新規**（rev12.3 §5.1 No.33）。AJAX |
...
## v1.6 で追加する Permission / Group / Role（rev19 §14）

【rev18 で改訂】 rev19 §14（CRUD ベース Permission ＋起点モデル基準グループ名）に追従。原則 CRUD ベース（Django 標準 `add/change/delete/view` を活用）とし、CRUD で表せない高リスク・固有操作のみ例外 Permission として手動定義する。グループ名は起点モデル基準（mailings 系＝`campaign_*`、tags 系＝`tag_*`）。

### Permission（mailings アプリ）

| Permission codename | content_type | 区分 | 説明 | 仕様書記載 |
|---|---|---|---|---|
| `mailings.view_campaign` | mailings.campaign | Django 標準 | 配信キャンペーン・配信レポートを閲覧できる（自分のもの） | ★1（rev19 §14.1.2） |
| `mailings.add_campaign` | mailings.campaign | Django 標準 | 配信キャンペーンを作成できる | ★1（rev19 §14.1.2） |
| `mailings.change_campaign` | mailings.campaign | Django 標準 | 配信キャンペーンを編集できる | ★1（rev19 §14.1.2） |
| `mailings.delete_campaign` | mailings.campaign | Django 標準 | 配信キャンペーンを削除できる（論理削除） | ★1（rev19 §14.1.2） |
| `mailings.send_campaign` | mailings.campaign | 例外（手動定義） | メール配信を実行できる（不可逆操作のため分離） | ★1（rev19 §14.1.2） |
| `mailings.view_all_campaigns` | mailings.campaign | 例外（手動定義） | 他人の配信キャンペーン・配信レポートも閲覧できる（横断権限） | ★1（rev19 §14.1.2） |
| `mailings.export_report` | mailings.campaign | 例外（手動定義） | 配信レポート CSV をダウンロードできる | ★1（rev19 §14.1.2） |
| `mailings.manage_template` | mailings.emailtemplate | 例外（手動定義） | メールテンプレートを作成・編集・削除できる（Campaign と 1:1 従属のため CRUD 化せず既存維持） | ★1（rev19 §14.1.2） |
| `mailings.view_suppressedemail` | mailings.suppressedemail | Django 標準 | 配信停止リストを閲覧できる | ★1（rev19 §14.1.2） |
| `mailings.manage_suppressed_email` | mailings.suppressedemail | 例外（手動定義） | 配信停止リストを編集できる（解除・手動追加、change 相当、既存維持） | ★1（rev19 §14.1.2） |
| `mailings.view_mailinglist` | mailings.mailinglist | Django 標準 | 配信リストを閲覧できる | ★1（rev19 §14.1.2） |
| `mailings.add_mailinglist` | mailings.mailinglist | Django 標準 | 配信リストを作成できる | ★1（rev19 §14.1.2） |
| `mailings.change_mailinglist` | mailings.mailinglist | Django 標準 | 配信リストを編集できる | ★1（rev19 §14.1.2） |
| `mailings.delete_mailinglist` | mailings.mailinglist | Django 標準 | 配信リストを削除できる | ★1（rev19 §14.1.2） |
| `mailings.change_mailingconfig` | mailings.mailingconfig（Django 標準 change 権限を流用） | Django 標準 | システムのメール配信設定を編集できる（admin 専用） | ★1（rev19 §14.1.2） |

### Permission（tags アプリ）

| Permission codename | content_type | 区分 | 説明 | 仕様書記載 |
|---|---|---|---|---|
| `tags.view_tag` | tags.tag | Django 標準 | タグの一覧・詳細を閲覧できる | ★1（rev19 §14.1.3） |
| `tags.add_tag` | tags.tag | Django 標準 | タグを作成できる | ★1（rev19 §14.1.3） |
| `tags.change_tag` | tags.tag | Django 標準 | タグを編集できる | ★1（rev19 §14.1.3） |
| `tags.delete_tag` | tags.tag | Django 標準 | タグを削除できる（論理削除） | ★1（rev19 §14.1.3） |
| `tags.assign_tag` | tags.tag | 例外（手動定義） | コンタクト / Person にタグを付与・解除できる | ★1（rev19 §14.1.3） |

【rev18 で改訂】 旧 `tags.create_tag` / `tags.edit_tag` / `tags.delete_tag` は廃止し、Django 標準 `add_tag` / `change_tag` / `delete_tag` に置き換えた（rev19 §14.1.3）。

### Group（v1.6 で追加、起点モデル基準）

| Group 名 | 含まれる Permission | 含まれる Role | 仕様書記載 |
|---|---|---|---|
| `campaign_admin` | `view_campaign`, `add_campaign`, `change_campaign`, `delete_campaign`, `send_campaign`, `view_all_campaigns`, `export_report`, `manage_template`, `view_suppressedemail`, `manage_suppressed_email`, MailingList 全（`view/add/change/delete_mailinglist`）, `change_mailingconfig` | `admin` | ★1（rev20 §14.2.1） |
| `campaign_editor` | `view_campaign`, `add_campaign`, `change_campaign`, `delete_campaign`, `send_campaign`, `export_report`, `manage_template`, MailingList 全（`view/add/change/delete_mailinglist`）, `view_suppressedemail`（閲覧のみ） | `sales` | ★1（rev19 §14.2.1） |
| `campaign_viewer` | `view_campaign`, `view_mailinglist`, `view_suppressedemail` | `viewer` | ★1（rev20 §14.2.1） |
| `tag_admin` | `view_tag`, `add_tag`, `change_tag`, `delete_tag`, `assign_tag` | `admin` | ★1（rev19 §14.2.2） |
| `tag_editor` | `view_tag`, `assign_tag` | `sales` | ★1（rev19 §14.2.2） |
| `tag_viewer` | `view_tag` のみ | `viewer` | ★1（rev19 §14.2.2） |

### Role の default_groups 拡張（rev19 §14.3）
...
| 2026-06-05 | **rev19→rev20：change_mailingconfig の誤分類是正（仕様書 rev21 §14.1.2 準拠）**：(1) 巻末「Permission（mailings アプリ）」表の `mailings.change_mailingconfig` 行の区分を「例外（手動定義）」→「Django 標準」に修正、content_type 注記を「mailingconfig（シングルトン、Meta 置き場所は実装時要確認）」→「mailingconfig（Django 標準 change 権限を流用）」に修正。(2) No.60（SystemSettingsView）備考末尾の「codename の置き場所＝シングルトン実体モデル（MailingConfig）の Meta は実装時に要確認」を「Django 標準 change_mailingconfig を流用（Meta に手書きしない）」に是正。Phase 7 のコード調査で change_mailingconfig は Django が自動生成する標準 change 権限であり、`Meta.permissions` に手書きすると auth.E005 で builtin と重複しシステムチェックが落ちることを確認したことに追従。codename（change_mailingconfig）・URL・ロール（admin）・★区分、および send_campaign 等の真の例外権限の区分は不変。 |
| 2026-06-03 | **認可改訂の追従（rev18→rev19）／仕様書 rev20 準拠**：仕様書 rev20 で view_suppressedemail を全ロール閲覧（admin/sales/viewer）に割り当てたことに追従。(1) No.57（SuppressedEmailListView, GET）の Permission を `mailings.view_campaign` → `mailings.view_suppressedemail`（ロール「全員」は不変）。(2) No.58（SuppressedEmailDetailView）の GET Permission を `mailings.view_campaign` → `mailings.view_suppressedemail`（POST=manage_suppressed_email・ロールは不変）。(3) 巻末 Group 定義表の campaign_admin に `view_suppressedemail` を追加、campaign_viewer を `view_campaign, view_mailinglist, view_suppressedemail` に追従。正本参照を仕様書 rev20 に更新。No.59（CreateView）・URL・パス・他の codename は不変。 |

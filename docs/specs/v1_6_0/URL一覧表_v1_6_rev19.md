...
- **`仕様書_v1_6_メール配信_クリックトラッキング_rev20.md` §14（v1.6 認可設計：CRUD ベース Permission ＋起点モデル基準グループ名、view_suppressedemail を全ロール閲覧に割り当て。本 rev19 で追従反映）**
...
| No. | ルート | メソッド | View 名 | URL name | ログイン | Permission | ロール | 備考 |
|---|---|---|---|---|---|---|---|---|
| 60 | `/settings/system/` | GET / POST | SystemSettingsView | `mailings:settings_system` | 必須 ★1 | `mailings.change_mailingconfig` ★1 | admin のみ ★1 | **v1.6 新規**（rev12.3 §5.1 No.38）。Settings モデル編集（シングルトン、特電法対応の会社情報・差出メール・DKIM 設定等、rev12.3 §4.13）。Permission は rev19 §14.1.2 で `mailings.change_mailingconfig`（admin 専用）に確定（旧★2 仮称 `mailings.manage_settings` から確定済みに更新）。codename の置き場所＝シングルトン実体モデル（MailingConfig）の `Meta` は実装時に要確認 |
...
| `mailings.change_mailingconfig` | mailings.mailingconfig（シングルトン、Meta 置き場所は実装時要確認） | 例外（手動定義） | システムのメール配信設定を編集できる（admin 専用） | ★1（rev19 §14.1.2） |

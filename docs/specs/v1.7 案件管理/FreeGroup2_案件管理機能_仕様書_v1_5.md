# FreeGroup2 案件管理（Deal）・活動記録（Activity）・会社（Company）機能 仕様書 v1.5

**作成日**：2026-09-06
**位置づけ**：v1.4に対し、ジェミニ君のレビューで得た実装指示書向けの申し送り事項を1点追加した版（`calculate_company_match()`のキーワード引数強制）。モデル構造・権限・アクセス制御・admin保護・Company重複検出のスコア/ランクロジック等、v1.4までに確定した内容はv1.5でも変更なし。本編仕様書（v1.6.3）・メール配信/クリックトラッキング仕様書（v1.6 rev21）と同じく、正式版として運用する。**本版をもってコード君向け実装指示書の作成に着手する。**
...
| `Deal.primary_person` → Person | `deals_as_primary` | `person.deals_as_primary.all()` |
| `DealPerson.person` → Person | `deal_persons` | `person.deal_persons.all()` |
...
**null/blank指定を明記**：商談の初期段階（初回商談直後等）では金額・受注予定日等が未定であることが普通にあり得るため、`name`／`primary_person`（相手方）以外は登録時点で未入力を許容する。これを明記しないと、コード君が全フィールドを登録時必須で実装し、`close_deal()`の`clean()`検証（§2.4）が無関係な項目で失敗する事故につながる。
...
| 主担当者 | primary_person | FK(Person, PROTECT) | 案件の相手方。必須 |
| 会社 | company | FK(Company, PROTECT, null=True) | §6章参照。**★v0.9：作成時に`primary_person.company`から自動補完する**（後述） |
| 担当者 | owner | FK(User, SET_NULL, null=True) | 現在の担当者。付け替え可能（§2.5参照） |
...
**★v0.9で確定：`Deal.company`の自動補完ルール**。§6.2で定義した`Person.company`プロパティ（`primary_contact.company`を返す読み取り専用）の使い道が未定義だったため、ここで確定する。

- **Deal作成時、`company`が未指定なら`primary_person.company`を初期値として設定する。** 案件の相手が決まれば会社も決まるのが通常であり、毎回選ばせるのは手間になるため
- **フォーム上で上書き可能とする。** 親会社と契約するが窓口は子会社の担当者、というケースが実際にあるため、自動補完はあくまで初期値である
- **作成後は追随させない。** `primary_person`の`primary_contact`が転職等で別会社に変わっても、`Deal.company`は変更しない。案件は「その時点でどの会社との商談だったか」の記録であり、相手の転職で過去の案件の相手先会社が書き換わるのは誤りである。担当者の転職に伴い案件そのものを引き継ぐ場合は、人が明示的に`company`を変更する
- **`primary_person`を後から変更した場合も、`company`は自動変更しない**（同じ理由）。画面上に「相手方の会社と案件の会社が異なります」という注意表示を出すに留める

**★v0.16で追加：実現方式はバックエンドの`form_valid`で行う（JS等の動的UIにしない）**。上記の「未指定なら初期値を設定する」を素直に実装しようとすると、「`primary_person`を選んだ瞬間に`company`のプルダウンをJS/HTMXで動的に書き換える」という複雑なUIに走りがちだが、**採らない**。

```python
# deals/views.py
class DealCreateView(...):
    def form_valid(self, form):
        if not form.instance.company_id and form.instance.primary_person:
            form.instance.company = form.instance.primary_person.company
...
画面側では、`lead_source`で「メールキャンペーン」を選択したときのみ`source_campaign`の入力欄を有効化する。§3.5.2のフォロー漏れ検出画面からDeal作成へ遷移する動線では、両方を初期値としてセットする。
...
**★v0.15で確定：Deal本体の通常編集フォームから`owner`・`primary_person`を除外する（重要）**。v0.14までのレビューで、「§5.1の`clean()`はDealPerson／DealUser作成・更新時にしか働かず、Deal編集フォームで`owner`／`primary_person`を直接書き換えた場合には再検証されない」という指摘があった。

これは`clean()`の限界の指摘としては正しいが、対策は「Deal本体の`clean()`に衝突チェックを足す」ことではなく、**そもそも通常編集フォームで`owner`・`primary_person`を直接変更できないようにする**ことで解決する。理由は2つある。
...
**Deal編集フォーム（`DealUpdateView`、`change_deal`権限で開く通常の編集画面）のフォームフィールドから`owner`・`primary_person`を除外する。** 画面には現在の値を読み取り専用で表示し、変更したい場合は専用の付け替え操作（本節の`reassign_deal_owner()`、次項の`reassign_deal_primary_person()`）に誘導する。この結果、§5.1の`clean()`が守る前提（`owner`／`primary_person`の変更は専用関数を必ず経由する）が構造的に保証され、通常編集フォームを経由した迂回が起こらない。
...
## 2.6.1 primary_person付け替え（★v0.15で新設）

`Deal.primary_person`（案件の相手方）の変更は、「案件がどの相手との商談か」という事実そのものを書き換える操作であり、`owner`の付け替えと同じ重みを持つ。担当者の異動・組織変更で相手方の窓口が変わるケースは実際に起こり得るため、変更手段自体は用意するが、`owner`と同じ理由で**専用の関数を経由させる**。

```python
def can_reassign_deal_primary_person(user, deal):
    """primary_person付け替えの権限判定。can_reassign_deal_owner と同型とする
...
def reassign_deal_primary_person(deal, new_primary_person, user):
    """primary_person付け替えの実行。呼び出し前に
    can_reassign_deal_primary_person(user, deal) のチェック（Service層）と、
    Viewの change_deal 権限（View層）が前提。

    reassign_deal_owner() と同型：new_primary_person が既に DealPerson として
    登録されている場合、その DealPerson レコードを削除してから昇格させる
    （§5.1の DealPerson.clean() が二重登録を禁じているため）。
    """
    with transaction.atomic():
        old_primary_person_id = deal.primary_person_id

        deal.deal_persons.filter(person=new_primary_person).delete()

        deal.primary_person = new_primary_person
        deal.updated_by = user
...
primary_person付け替えの画面も、owner付け替えと同じく専用のフォーム・エンドポイントとする（`permission_required = 'deals.change_deal'`）。`Deal.company`（§2.1）は`primary_person`変更時に自動追随させない方針を既に確定しているため（相手の転職で過去の案件の会社が書き換わるのは誤りであるため）、この関数もその方針をそのまま踏襲し、`company`には触れない。
...
**DealPerson**：`deal`（FK, CASCADE）／`person`（FK, CASCADE）／`role`（CharField(max_length=50, choices=PersonRole.choices)、**§2.9.4参照**）／`memo`（CharField(max_length=255, blank=True)）／`created_at`
...
**★v0.14で追加：本体の単一FKとの二重登録を防ぐ`clean()`（重要）**。§5.2は初版から「`Deal.primary_person`／`Deal.owner`が『正』であり、DealPerson／DealUserには同一人物を二重登録しない（バリデーションで縛る）」と宣言してきたが、そのコードが一度も示されていなかった。§5.1の`UniqueConstraint`が防ぐのは「同じ人を2回登録すること」であり、「`owner`本人を`DealUser`にも登録すること」は別物で、これでは防げない。
...
def clean(self):
    super().clean()
    if self.person_id == self.deal.primary_person_id:
        raise ValidationError(
...
`DealPerson`は「相手方」という単一の特別枠（`primary_person`）を持つのに対し、`Activity`は`person`単体のFKを持たない（§3.1、複数の相手と同時に接触することが前提）ため、`ActivityPerson`側には対応する二重登録チェックが存在しない。この非対称は意図的である。

## 5.2 設計原則

- `Deal.primary_person`／`Deal.owner`（本体の単一FK）が「正」。DealPerson／DealUserには同一人物を二重登録しない（バリデーションで縛る）
...
しかし、この保護は**すべてView層のフォーム構成に依存している**（例：Deal編集フォームから`owner`・`primary_person`を除外する、§2.6）。既存の全アプリ（`persons`／`cards`／`contacts`／`tags`／`mailings`／`duplicates`／`accounts`／`actionlogs`）は`admin.py`を持っており、新設4アプリにも`admin.py`が登録されるのはほぼ確実である。**Django admin（`ModelAdmin`）はデフォルトで全フィールドを直接編集可能にするため、`readonly_fields`等で明示的に塞がない限り、admin画面はこれらの保護を丸ごと迂回する。**
...
| `Deal.primary_person` | 同上（`reassign_deal_primary_person()`を迂回、`DealPerson.clean()`に阻まれる） |
...
**★v0.18で修正：静的な`readonly_fields`ではなく`get_readonly_fields()`を使う（重要）**。Djangoの`readonly_fields`はクラス属性として静的リストで指定すると、**「追加（Add）」フォームと「変更（Change）」フォームの両方に同じリストが適用される**。`Deal.primary_person`は§2.1で`FK(Person, PROTECT)`の必須項目（デフォルト値なし）であり、これが常にreadonlyだと、**admin画面から新規Dealを作成すること自体ができなくなる**（必須項目が未入力のまま保存しようとしてバリデーションエラーになる）。`Company.status`も同様の懸念があり、モデル側にデフォルト値がなければ、常にreadonlyにすると§6.6が言及する「Company新規作成画面から人が作る」経路がadmin側では詰まる（`Company.status`のデフォルト値については後述、§6.2で対応済み）。
...
class DealAdmin(admin.ModelAdmin):
    def get_readonly_fields(self, request, obj=None):
        if obj is None:  # 追加時は制限しない（primary_personは必須項目のため）
            return []
...
| Deal.primary_person／DealPerson.person／Activity（ActivityPerson経由） | **付け替えない**。読み取り時解決 | 進行中の商談だが、Personには**復元機能がある**ため、付け替えると復元時に戻す処理（`previous_person`追加）が必要になり複雑化する |
...
deal.primary_person.get_surviving_person()
```

一覧画面のN+1対策：`Deal.objects.select_related('primary_person', 'primary_person__merged_into')`。

**★v0.8で明記：多段マージ時の追加クエリ（既知の許容事項）**。`select_related('primary_person__merged_into')`がカバーするのは1ホップ分のみであり、A→B→Cのような多段マージが存在する場合、`get_surviving_person()`の2ホップ目以降で追加クエリが発生する。§3.5.2のフォロー漏れ検出も同じ性質を持つ。実運用でPersonマージが3段以上連鎖するケースは稀であり、対策するには既存の共有メソッド`get_surviving_person()`の改修（あるいは生存Person IDの非正規化キャッシュ）が必要になるため、**本フェーズでは既知の許容事項として受容する**。
...
- CreateViewの画面構成（本体のみ登録 → 詳細画面へリダイレクト → サブフォームで関与者・添付を追加する2段構え。巨大なinlineformsetを作らない）
- **Deal編集フォーム（`DealUpdateView`）のフォームフィールドから`owner`・`primary_person`を除外すること**（★v0.15で追加。§2.6参照）。除外を怠ると、通常編集フォーム経由でDealPerson／DealUserとの二重登録が静かに発生する
- `Deal.owner`付け替え時に、新ownerが既にDealUserなら当該DealUserを削除して昇格させる（§5.2の二重登録禁止との整合）
- 日報一覧のGETパラメータ（`date`／`user`）の不正値フォールバック（本日／自分）
- admin登録と`autocomplete_fields`、`CompanyAdmin.search_fields`。**Service関数を経由すべきフィールドの`readonly_fields`指定は§7.6.1を参照（実装段取りの範囲外、業務ルールの保護そのものであるため仕様書側に明記済み）**。**★v1.0で追記：admin.py登録時のチェックリストとして、対象を問わず「Service関数を経由すべきフィールド・アクションを`readonly_fields`／`has_delete_permission`で塞ぐ」ことを徹底する。該当：`Deal.owner`／`primary_person`／`is_archived`（§7.6.1）、`Activity.is_archived`（§7.6.1）、`Company.status`／`merged_into`（§7.6.1）、**`AttachmentAdmin`の削除操作**（`post_delete`シグナルで実ファイルは消えるが、`ActionLog`記録は`delete_attachment()`経由でしか行われないため、`has_delete_permission()`を`False`にしてadmin経由の削除自体を禁止する。§4.4.2参照）。この4種類はいずれも同一パターン（Service関数を経由する重要操作を、admin登録時に個別に塞ぎ忘れる）であり、実装時にadmin.pyを書く段階でこのチェックリストを一度通しで確認すること
...
| 主担当者（相手方） | primary_person | 成約確定日 | closed_at |
| 会社 | company | 案件発生源 | lead_source |
| 担当者 | owner | 発生源キャンペーン | source_campaign |
| 作成者 | created_by | 失注理由 | lost_reason |
| 更新者 | updated_by | 概要メモ | memo |
| ステージ | stage | 論理削除フラグ | is_archived |
| 確度 | probability | 作成日時 | created_at |
| 種別 | deal_type | 更新日時 | updated_at |
| 金額 | amount | | |

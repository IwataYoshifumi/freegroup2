# FreeGroup2 案件管理（Deal）・活動記録（Activity）・会社（Company）機能 仕様書 v1.15

**作成日**：2026-09-15
**位置づけ**：v1.14は実装着手前レビュー（2名）で「LGTM・完全FIX（凍結）」の判定を受けた。v1.15はその際の申し送り1点（`_normalize_url_string()`単体呼び出し時の防御）のみを反映した最終確定版。業務ルール・スコア・ランク判定・画面仕様に変更はない（防御的コーディングの追加のみ）。
...
## v1.14からの主な変更点（★実装着手前レビューで完全LGTM、防御的コーディング1点のみ追加して最終確定）

実装着手前の2名によるレビューで、v1.14は「ロジック変更なしの純粋なリファクタリングとして正しく機能している」として**完全LGTM（完全FIX・凍結）**の判定を受けた。以下1点の申し送りのみを反映し、v1.15を最終版とする。

- **`_normalize_url_string()`に単体呼び出し時の防御を追加**：現在の呼び出し順序（`_extract_netloc()`・`normalize_website()`経由）では`None`や空文字列が`_normalize_url_string()`に直接渡ることはないが、将来の単体テストや別関数からの直接呼び出しに備え、関数冒頭に`if not url: return ''`のガードを追加した（§6.5.1）

## v1.13からの主な変更点（★実装着手前レビューでLGTM、軽微な指摘1点のみ解消して確定）

実装着手前の2名によるレビューで、v1.13は「住所前方一致バグの解消」「`url_mismatch`のnetloc純化」のいずれも的確に是正されているとして**LGTM（合格）**の判定を受けた。仕様書はv1.13の内容のまま**FIX（凍結）**とし、以下の軽微な指摘1点のみを反映してv1.14とする。

- **`normalize_website()`と`_extract_netloc()`のnetloc抽出ロジックの重複を解消**：両関数とも「URL文字列の前処理（strip・lower・スキーム補完）→`urlparse()`→`netloc`の`www.`除去」という同一のコードを別々に持っていた。実害はないが保守性の観点から、`normalize_website()`の内部で`_extract_netloc()`を呼び、その結果にパスの第1セグメントを付け足す形に整理した（§6.5.1）

コード君向け実装指示書を作成する際は、以下のモジュール配置を明記する（レビューでの申し送り）。

- `_check_address_match`：`companies/services.py`内の既存関数
- `normalize_phone_value`：`contacts/services/normalization.py`からimport
- `_extract_netloc`：`companies/services.py`内に新設（`normalize_website`・`calculate_company_match`双方から利用）

## v1.12からの主な変更点（★即死級バグ：住所前方一致の実装ミス／URL不一致判定の設計改善／ドキュメント表記ズレの是正）

実装着手前の2名によるレビューで、v1.12にさらに3点の問題が判明した。うち1点は「v1.11で防ごうとした誤結合が別の形で大量再発する」即死級のバグだった。

- **①【最重要・即死級バグ】住所の前方一致が「先頭5文字スライス比較」になっていた欠陥を修正**：v1.12で追加した住所比較コードは`normalized_address_a[:5] == normalized_address_b[:5]`（先頭5文字が一致すればTrue）という実装になっていた。日本の住所は先頭5文字が「都道府県名＋市区町村名」に相当することが多く（例：「愛知県豊田」「東京都渋谷」）、この実装では**同じ市区町村内にある無関係な別住所同士が住所一致（+20点）を獲得してしまう**。ドメイン一致（100点）を持つCATV共用ドメインや親会社共用ドメインのケースで、この住所加点が加わると`possible_low`（120点）に到達し、v1.11で足切りしたはずの誤結合が住所経由で大量に復活する重大な事故だった。**是正**：本来意図されていた「一方の住所が他方の前方一致になっている（ビル名・部屋番号の有無を救済する）」という判定に修正した。あわせて、実際のコードベース（`companies/services.py`）を確認した結果、この前方一致判定は既に`_check_address_match()`として実装済みであることが判明したため、新規にコードを書かず既存関数を呼ぶ形に修正した
- **②【設計改善】電話・住所の正規化は既存関数の呼び出しに統一**：v1.12では「電話・住所の比較が生文字列完全一致のままだった」という前提で正規化コードを新規に書き起こしたが、レビューで実際のコードベース（`companies/services.py`・`contacts/services/normalization.py`）を確認した結果、電話は既存の`normalize_phone_value()`（全角数字・漢数字の半角化＋数字以外除去）、住所は上記`_check_address_match()`が**既に実装済み**であることが判明した。v1.12の新規インライン実装はいずれもこれら既存関数より機能が後退していた（電話：全角数字・漢数字の変換が漏れる／住所：①の即死級バグ）。**是正**：新規コードを削除し、既存の`normalize_phone_value()`・`_check_address_match()`をそのまま呼ぶ形に修正した（§6.5.1）。実装着手前レビューの本来の目的（仕様と実コードの食い違いを潰す）に即し、今回は仕様書側が実コードより古い前提を置いてしまっていた
- **③【設計改善】`url_mismatch`の判定基準を「ドメイン（netloc）自体の食い違い」に純化**：v1.12は「同じ会社の名刺2枚のURLがトップページと下層ページで異なるだけで`url_mismatch`が誤発動する」問題に対し、`determine_company_rank()`側で「`name_match`が真の場合はこの足切りをスキップする」という対症療法を採った。しかしこの例外には別の穴があった：全国に多数存在する同名企業（例：「株式会社田中製作所」東京支店・大阪支店ではなく無関係な同名の別法人）が、CATV等の共用ドメイン（+100点）と会社名一致（+100点）で200点に達した場合、`name_match=True`であるため`url_mismatch`足切りが無条件にスキップされ、ドメインが全く異なる別法人であっても`possible_high`（住所加点次第では`exact_match`）に誤昇格してしまう。**是正**：問題の根本原因は「下層ページ違いを、ページ違いのレベル（パスまで含めた正規化）で食い違い判定していたこと」にあったため、`url_mismatch`の定義自体を「正規化後URL（ドメイン＋パス）の食い違い」から「**Webサイトのドメイン（netloc）自体の食い違い**」に変更した。同一ドメイン内のパス違い（トップページと下層ページ）はそもそも`url_mismatch`ではなく単なる`url_match=False`（加点なしだが足切りもなし）として扱われるようになったため、`determine_company_rank()`側の`not name_match`という危うい迂回路が不要になり、v1.11の原案どおりの単純な`if url_mismatch: return None`に戻した。これにより「同一会社の下層ページ違いによる見逃し」と「同名他社のドメイン違いによる誤結合」の両方を、判定関数側に特例を持たせることなく解消した（§6.5.1・§6.5.2）
- **④ドキュメントの表記ズレを修正**：v1.12で`has_backing`から`url_match`を除去した際、§6.5.2の`possible_low`必須条件テーブルの記述「(電話一致 OR 住所一致 OR URL一致)のいずれか1項目以上が必須」が古いまま残っていた。コードと一致するよう「(電話一致 OR 住所一致)のいずれか1項目以上が必須」に修正した
- **⑤`_build_duplicate_groups()`のグループオブジェクトの構造を補足**：コード例中の`group.candidates`・`g.max_score`が何を想定したオブジェクトかが示されていなかったため、簡単な説明コメントを追加した（§6.5.3.1）

## v1.11からの主な変更点（★実装着手前レビューで判明した5点を反映：url_mismatch誤爆・電話/住所正規化欠落・index.html誤検出ほか）

実装着手前の2名によるレビューで、v1.11の設計上の欠陥・実装バグ化リスクが5点判明した。

- **①【最重要】`url_mismatch`足切りが会社名完全一致を無条件に握りつぶす欠陥を修正**：v1.11の`determine_company_rank()`は`name_match`を一切見ずに`url_mismatch`を最優先で足切りしていた。しかし`normalize_website()`は「ドメイン＋パスの最初のセグメント」までしか見ないため、同じ会社の名刺2枚のURLがトップページ（`/`）と下層ページ（`/products/`等）で異なるだけで`url_mismatch=True`になり得る。この場合、会社名・ドメインが完全一致し220点を超えていても、無条件に候補外（`None`、DBにすら記録されない）にされ、「自動リンクされるべき明白な重複が誰にも見えない形で永久に見逃される」という、今回の是正の目的（誤結合を減らす）とは逆方向の見逃し事故を生む。これは仕様書自身が明記する「会社名一致は`exact_match`／`possible_high`の必須条件から絶対に外さない」という設計思想とも矛盾していた。**是正**：`url_mismatch`による足切りは`name_match`が偽の場合のみ発動するよう条件を絞った（§6.5.2）
- **②`calculate_company_match()`の電話・住所比較が生の文字列完全一致のままだった欠陥を修正**：§6.5.1のスコア表は「電話：数字のみに正規化した後の完全一致」「住所：空白除去後、完全一致または5文字以上の前方一致」と定めていたが、直下のコード例は`phone_a == phone_b`／`address_a == address_b`という正規化なしの比較のままだった。v1.11で`possible_low`に「電話または住所の追加裏付け必須」を導入したため、このバグを放置すると「0565-12-3456」と「0565123456」のようなハイフン有無だけで`phone_match`が`False`になり、本来救済すべき「社名変更後も同一メールドメイン＋同一拠点」のケースが軒並み足切りされる実害があった。**是正**：`calculate_company_match()`内に数字抽出・空白除去・前方一致の正規化コードを明記した（§6.5.1）
- **③`normalize_website()`のトップページファイル名誤検出を是正**：`https://example.co.jp/`と`https://example.co.jp/index.html`は同一会社の同一トップページを指すが、パスの最初のセグメントをそのまま比較キーに使うと`example.co.jp`と`example.co.jp/index.html`という別物として扱われ、`url_mismatch`が誤って発動する。**是正**：`index.html`／`index.htm`／`index.php`等の一般的なトップページファイル名をセグメント抽出から除外するガードを追加した（§6.5.1）
- **④§6.5.4のプレフィルタの仕様上の割り切りを明記**：`possible_mid`は`(会社名一致 OR ドメイン一致 OR URL一致)`のいずれか1つで140点以上に到達できるが、§6.5.4のSQLプレフィルタは`organization`・`domain`の2条件のみで絞り込んでいるため、「社名不一致・ドメイン未設定・URL一致＋電話・住所一致（計140点）」というpossible_mid該当ケースは、プレフィルタの時点で0件になりPythonのスコア計算にすら到達しない。URLに索引を張るとフルスキャンになり重いため、この割り切りは意図的な性能対策として維持するが、不具合と誤解されないよう§6.5.4に既知の制約として明記した
- **⑤`_build_duplicate_groups()`のソート優先度定義とデッドコードの整理**：ソート処理が参照する`_group_rank_priority()`の戻り値の数値体系が示されておらず、実装者が「優先度1＝最優先」と誤解して符号を逆に付けるリスクがあったため、`RANK_PRIORITY`マッピング（`exact_match`／`possible_high`＝3、`possible_mid`＝2、`possible_low`＝1）を明記した（§6.5.3.1）。あわせて、`determine_company_rank()`の`has_backing`が参照する`url_match`は、`url_match=True`かつ`(name_match or domain_match)=True`の場合スコアが必ず200点以上になり`possible_low`到達前に上位ランクで`return`されるため実質デッドコードだった点を整理し、`has_backing = phone_match or address_match`に簡素化した（§6.5.2）

## v1.10からの主な変更点（★Company重複判定の是正：単独一致の足切り・possible_lowの是正・レビュー画面のグルーピング明文化）

実データ（名刺インポート・会社リンク）運用において、以下2つの深刻な誤結合事象が確認された。

- **地域CATV・プロバイダ共用ドメインによる誤結合**：`aitai.ne.jp`（ひまわりネットワーク等の地域CATV）を利用する無関係な中小企業同士が、ドメイン一致（+100点）のみで重複候補（`possible_high`、200点）として束ねられていた。除外リストへの追加で一時対応してきたが、未知の共用ドメインが現れるたびに設定を追加し続ける「モグラ叩き運用」の限界が露呈した
- **グループ企業・親族企業（共用独自ドメイン）の巻き込み**：「ノックス電子株式会社」が親会社「野場電工株式会社」の独自メールドメイン（`noba.co.jp`）を社員メールとして共用していたケースで、v1.10の仕様（社名不一致・ドメイン一致＝`possible_low`、100点）では、社名・電話・住所・URLがすべて異なる無関係な別会社が重複候補レビュー画面に抽出された。さらに全ランクを一括で連結するUnion-Findであったため、野場電工同士の`possible_high`グループ（200点）にノックス電子（100点）が巻き込まれ、1つの巨大カードとして誤表示された

これらを踏まえ、以下の是正を行った（§6.5.1・§6.5.2・§6.5.3）。

- **単独一致（追加裏付けなしの会社名のみ／ドメインのみ／URLのみ、各100点）の候補化を完全廃止し、足切り（候補外）とする**。社名単独一致は従来から候補外だったが、ドメイン単独一致・URL単独一致も同様に候補外とする。これにより、未知のCATVや親会社共用ドメインが混入しても、社名・電話・住所の裏付けがなければレビュー画面に一切浮上しなくなる
- **`possible_low`の閾値を140点未満から「120点以上」に是正し、必須条件に「電話・住所・URLのいずれか1項目以上の追加一致」を新設**：条件は`(会社名一致 OR ドメイン一致)` AND `(電話一致 OR 住所一致 OR URL一致)`とする。これにより「社名変更があったが同一メールドメインかつ同一拠点」という実務上あり得るケースは救済しつつ、ドメイン共用だけの無関係な別会社は排除する
- **Webサイト（URL）の不一致足切りを新設**：比較元の双方が`website`を保持しており、正規化後のURLが明確に異なる場合は、メールドメインが同一であってもグループ会社・別法人とみなし、候補外とする
- **重複候補レビュー画面のランク別独立グルーピングを明文化**：`_build_duplicate_groups()`は全ランク一括のUnion-Findを禁止し、`exact_match`／`possible_high`、`possible_mid`、`possible_low`のランクごとに独立してUnion-Findを実行する（§6.5.3.1、新設）
- **重複候補レビュー画面への「HP（URL）」列常設を明文化**：スコア判定に使う5要素（社名・ドメイン・代表電話・HP・住所）をすべてテーブル上で比較できるようにする（§6.5.3.1、新設）
...
| フィールド | スコア | 判定条件・正規化仕様 |
|---|---|---|
| ドメイン（`domain_match`） | 100（★v1.3で120から変更） | `org_domain_name`と`Company.domain`の完全一致。両者とも`is_generic_email_domain()`で汎用ドメインでないこと必須 |
| 会社名（`name_match`） | 100 | **★v1.11で明確化**：`organization`の生の完全一致ではなく、既存の`normalize_organization()`（`contacts/services/normalization.py`、実装済み、前株・後株・法人格・全角半角・英大文字小文字を正規化する純関数）を通した後の完全一致とする。新規に正規化関数は作らない |
| URL（`url_match`） | 100（★v1.3で20から変更） | `website`を`normalize_website()`（後述）で正規化した後の完全一致 |
| 電話一致（`phone_match`） | 20 | `org_phone`と`Company.phone`を`normalize_phone_value()`（既存、全角数字・漢数字の半角化＋数字以外除去）で正規化した後の完全一致 |
| 住所一致（`address_match`） | 20 | `address`と`Company.address`を`_check_address_match()`（既存、空白除去後に完全一致または一方が他方の前方一致・5文字以上必須）で判定 |
...
**★v1.11で新設：足切り・除外ルール（実データ運用で確認された2つの誤結合事象への対処）**

実運用で、以下2つの深刻な誤結合が確認された。

1. **地域CATV・プロバイダ共用ドメインによる誤結合**：`aitai.ne.jp`（ひまわりネットワーク等の地域CATV）を利用する無関係な中小企業同士が、ドメイン一致（100点）のみで`possible_high`（200点）として束ねられていた。除外リストへの追加で一時対応してきたが、未知の共用ドメインが現れるたびに設定追加を繰り返す「モグラ叩き運用」の限界がある
2. **グループ企業・親族企業（共用独自ドメイン）の巻き込み**：子会社が親会社の独自メールドメインを社員メールとして共用しているケースで、社名・電話・住所・URLがすべて異なる無関係な別会社が、ドメイン一致（100点）だけで`possible_low`として重複候補に上がっていた

この2点に対処するため、以下2つの足切りルールを新設する。

- **① 単独一致の完全排除**：追加の客観的裏付け（電話・住所・URL・会社名のいずれか）が一切ない「ドメイン単独一致（100点）」「URL単独一致（100点）」「会社名単独一致（100点）」は、いずれも**候補外（`None`）とし、`CompanyDuplicateCandidate`をDBに保存しない**。会社名単独一致は従来（v1.0）から候補外だったが、v1.11でドメイン単独一致・URL単独一致も同列に足切りする。これにより、未知のCATVや親会社共用ドメインが混入しても、社名・電話・住所のいずれの裏付けもなければレビュー画面に一切浮上しなくなる
- **② Webサイト（URL）の不一致足切り**：比較元の双方が`website`を保持しており、Webサイトの**ドメイン（netloc）自体**が明確に異なる場合は、メールドメインが同一であっても重複候補とせず**候補外（`None`）**とする。持株会社と事業会社のように同一メールドメインを共用しつつ公式サイトが別々に存在するケースを、ドメイン一致だけで誤って重複扱いしないための防衛策。**★v1.13で判定基準を是正**：v1.11〜v1.12は`normalize_website()`（ドメイン＋パスの第1セグメント）の食い違いを基準にしていたが、これだと同じ会社の名刺2枚のURLがトップページと下層ページで異なるだけで誤発動する問題があった（v1.12ではこれを`determine_company_rank()`側の`name_match`例外で回避したが、今度は「ドメインが全く異なる同名の別法人」が`name_match=True`を理由に足切りをすり抜けてしまう別の穴を生んだ）。v1.13では判定基準そのものを「正規化後URL全体」から「**ドメイン（netloc）自体**」に変更し、同一ドメイン内のパス違いは`url_match=False`（加点なし、足切りもなし）として扱うようにした。これにより、`determine_company_rank()`側に`name_match`例外という特例を持たせる必要がなくなり、v1.11の原案どおり無条件の足切りに戻せる（詳細は`calculate_company_match()`のdocstring・§6.5.2参照）

具体的な判定への反映は§6.5.2（ランク判定）の`determine_company_rank()`改訂を参照。
...
# ★v1.12で追加：一般的なトップページ用ファイル名。パスの最初のセグメントが
# これに該当する場合は「実質パスなし＝トップページ」とみなし、除外する。
INDEX_FILENAMES = {'index.html', 'index.htm', 'index.php'}


def _normalize_url_string(url):
    """★v1.14で新設：URL文字列の前処理（前後空白除去・小文字化・スキーム
    補完）のみを行う。_extract_netloc()・normalize_website()の両方が使う
    共通の下ごしらえで、urlparse()に渡す直前の形に整える。

    ★v1.15で追加：現在の呼び出し順序ではNone・空文字列がここに直接渡る
    ことはないが（呼び出し元の_extract_netloc()・normalize_website()が
    先にガードしているため）、将来の単体テストや別関数からの直接呼び出し
    に備え、本関数自体にも防御を追加した（レビュー指摘）。
    """
    if not url:
        return ''
...
def _extract_netloc(url):
    """★v1.13で新設：URLからドメイン部分（netloc、www.除去済み）だけを
    取り出す純関数。url_mismatch判定（ドメイン自体のみを見る、後述）で
    使う。空文字列の場合は''を返す。
    """
    if not url:
        return ''
...
def normalize_website(url):
    ...
    url_match（積極的な一致材料、+100点）の判定に使う。
    ...
    ★v1.12で修正：https://example.co.jp/ と https://example.co.jp/index.html
    は同一会社の同一トップページを指すが、修正前はパスの最初のセグメントを
    そのまま比較キーにしていたため、後者が example.co.jp/index.html となり
    前者（example.co.jp）と食い違ってしまい、url_mismatch（§6.5.2）を
    誤って発動させていた。index.html／index.htm／index.php等の一般的な
    トップページファイル名はセグメント抽出から除外する。

    ★v1.14で整理（レビュー指摘、実害のない軽微な重複コードの解消）：
    netloc抽出部分を自前で再実装せず、_extract_netloc()をそのまま呼ぶ
    形にした。v1.13時点では両関数が「前処理→urlparse→netloc取得」という
    同一のコードを別々に持っており、保守性の観点で重複していた。
    本関数自体の挙動（ドメイン＋パス第1セグメントを返す）は変更なし。
    """
...
`Company.website`・`Contact.website`はどちらも生のURL文字列のまま保存する（正規化前の値を表示に使うため）。比較の直前に`normalize_website()`（url_match用）または`_extract_netloc()`（url_mismatch用）を通す。両関数は`companies/services.py`に配置する（`_check_address_match()`と同じモジュール）。
...
**★v1.13で明記：電話・住所の正規化は既存関数をそのまま呼ぶ。新規に作らない。**

- `normalize_phone_value(raw)`：電話番号1件の正規化（既存、`contacts/services/normalization.py`）。全角数字・漢数字を半角化した上で数字以外を除去する
- `_check_address_match(address_a, address_b)`：住所2件の一致判定（既存、`companies/services.py`）。空白除去後、完全一致または一方が他方の前方一致（`a.startswith(b) or b.startswith(a)`方式、前方一致は5文字以上必須）。「愛知県豊田市西町2-18-2」と「愛知県豊田市西町2-18-2 豊田ビル3F」は一致するが、「愛知県豊田市西町...」と「愛知県豊田市日之出町...」のような同一市区町村内の別住所は一致しない

いずれも実装済みのため、`calculate_company_match()`内で新規に正規化コードを書き起こす必要はない。
...
class CompanyMatchResult:
    ...
    ★v1.11で`url_mismatch`・`phone_match`・`address_match`を追加。
    `phone_match`・`address_match`は、possible_lowの「追加裏付け必須」
    判定（§6.5.2）でdetermine_company_rank()が直接参照できるよう、
    内部計算値をそのまま返り値に含めた（従来はスコア加算にのみ使い、
    返り値には含めていなかった）。

    `url_mismatch`は★v1.13で判定基準を変更：双方のwebsiteが保持する
    ドメイン（netloc）自体が食い違う場合のフラグ（v1.11〜v1.12時点は
    パスまで含めた正規化後URLの食い違いだったが、これだと同じ会社の
    名刺2枚がトップページと下層ページで異なるだけで誤発動していた
    ため、ドメインのみの比較に変更した。詳細はcalculate_company_match()
    のdocstring参照）。単なる「一致しない」（url_match=Falseかつ片方が
    空の場合を含む）とは区別し、双方にドメインがあってなお食い違う
    「明確な不一致」の場合のみdetermine_company_rank()側で足切りに使う。
    """
    score: int
    name_match: bool
    domain_match: bool
...
def calculate_company_match(organization_a, domain_a, phone_a, address_a, website_a,
    ...
    ★v1.11で2点改修。
    ①会社名の一致判定を、生の`organization`同士の完全一致から、既存の
    `normalize_organization()`（前株・後株・法人格・全角半角・英大文字小文字
    を正規化する純関数、実装済み）を通した後の完全一致に変更した
    （§6.5.1・実データで表記ゆれによる名寄せ漏れが確認されたため）。
    ②双方がwebsiteを保持しているにもかかわらず正規化後のURLが食い違う
    場合を`url_mismatch`として検出できるようにした（§6.5.1の足切り②）。

    ★v1.12で改修を試みたが、レビューで実コード（`companies/services.py`）
    との食い違いが判明し★v1.13で是正（詳細は下記参照）。

    ★v1.13で2点是正。
    ①電話・住所の比較を新規コードから既存関数呼び出しに変更した。v1.12は
    「電話・住所の比較が生の文字列完全一致のままだった」という前提で
    正規化コードを新規に書き起こしたが、実際のコードベースを確認した結果、
    電話は既存の`normalize_phone_value()`（`contacts/services/normalization.py`、
    全角数字・漢数字の半角化＋数字以外除去）、住所は既存の
    `_check_address_match()`（`companies/services.py`、空白除去後に
    完全一致または一方が他方の前方一致・5文字以上）が**既に実装済み**
    であることが判明した。v1.12の新規インライン実装はいずれもこれらの
    既存関数より機能が後退しており（電話：全角数字・漢数字の変換漏れ、
    住所：後述の即死級バグ）、新規コードを削除し既存関数を呼ぶ形に
    修正した。
    ②`url_mismatch`の判定基準を「正規化後URL（ドメイン＋パス）の食い違い」
    から「ドメイン（netloc）自体の食い違い」に変更した。詳細は
    `_extract_netloc()`および`determine_company_rank()`（§6.5.2）参照。
    """
    normalized_org_a = normalize_organization(organization_a)
    normalized_org_b = normalize_organization(organization_b)
    name_match = bool(normalized_org_a) and normalized_org_a == normalized_org_b

    domain_match = bool(domain_a) and domain_a == domain_b and not is_generic_email_domain(domain_a)

    # ★v1.13で是正：新規コードを書かず、既存の純関数をそのまま呼ぶ。
    # normalize_phone_value()は全角数字・漢数字を半角化した上で数字以外を
    # 除去する既存の共通正規化関数（contacts/services/normalization.py、
    # 実装済み）。新規に正規化ロジックを作らない。
    normalized_phone_a = normalize_phone_value(phone_a) if phone_a else ''
    normalized_phone_b = normalize_phone_value(phone_b) if phone_b else ''
    phone_match = bool(normalized_phone_a) and normalized_phone_a == normalized_phone_b

    # ★v1.13で是正（★重要）：v1.12は空白除去後に normalized_address_a[:5]
    # == normalized_address_b[:5]（先頭5文字が一致すればTrue）という
    # 実装になっていたが、日本の住所は先頭5文字が「都道府県名＋市区町村名」
    # に相当することが多く（例：「愛知県豊田」）、この実装だと同一市区町村内
    # の無関係な別住所同士まで一致扱いになってしまう即死級のバグだった
    # （レビューで指摘。ドメイン一致100点にこの誤った住所加点20点が乗ると
    # possible_lowの120点に到達し、v1.11で足切りしたはずの誤結合が
    # 住所経由で大量に復活する）。既存の`_check_address_match()`
    # （companies/services.py、実装済み。a.startswith(b) or b.startswith(a)、
    # 5文字以上必須の正しい前方一致）を呼ぶ形に修正した。新規に
    # 正規化ロジックを作らない。
    address_match = _check_address_match(address_a, address_b)
    ...
    # ★v1.13で是正：url_mismatchはパスまで含めた正規化ではなく、
    # ドメイン（netloc）自体の食い違いのみで判定する。v1.12まではURL全体
    # （ドメイン＋パス第1セグメント）で比較していたため、同じ会社の名刺
    # 2枚のURLがトップページ（/）と下層ページ（/products/等）で異なる
    # だけでもurl_mismatch=Trueになってしまい、v1.12ではこれを
    # determine_company_rank()側の`not name_match`という特例で回避して
    # いた。しかしこの特例には「同名の別法人が共用ドメインで一致した場合、
    # ドメインが全く異なっていてもname_match=Trueであるため足切りが
    # スキップされてしまう」という別の穴があった（レビューで指摘）。
    # 判定基準をドメイン自体に変更することで、同一ドメイン内のパス違い
    # （url_match=Falseになるだけで足切りにはしない）と、ドメイン自体が
    # 異なる別法人（name_matchの真偽に関わらず足切りする）を、
    # determine_company_rank()側に特例を持たせずに区別できる。
    netloc_a = _extract_netloc(website_a)
    netloc_b = _extract_netloc(website_b)
    ...
    return CompanyMatchResult(
        ...
        phone_match=phone_match, address_match=address_match,
    )
```
...
rank = determine_company_rank(
    ...
    match_result.url_mismatch, match_result.phone_match,
...
**★v1.11で全面是正：単独一致の完全排除・possible_lowの裏付け必須化・URL不一致足切り**。実データ運用で、ドメイン単独一致・URL単独一致（各100点）が地域CATV等の共用ドメインを候補化してしまう事象と、`possible_low`（v1.3〜v1.10：会社名・ドメイン・URLのいずれか一致＋100点以上）がグループ企業間の共用独自ドメインを誤って候補化する事象が確認された（§6.5.1参照）。これを踏まえ、下表・コードのとおり是正する。

**★v1.12で是正（★v1.13でさらに設計改善、下記参照）**：v1.11の`url_mismatch`足切りは`name_match`を一切見ずに最優先で発動する設計だったが、`normalize_website()`が「ドメイン＋パスの最初のセグメント」までしか見ないため、同じ会社の名刺2枚のURLがトップページ（`/`）と下層ページ（`/products/`等）で異なるだけで`url_mismatch=True`になり得る。この場合、会社名・ドメインが完全一致し220点を超えていても無条件に候補外（`None`、レビューキューにすら乗らない）にされてしまい、「自動リンクされるべき明白な重複が誰にも見えない形で永久に見逃される」という、この足切りが本来防ぐべき誤結合とは逆方向の見逃し事故を生んでいた（レビューで指摘）。v1.12ではこれに対し「`url_mismatch`による足切りは`name_match`が偽の場合のみ発動する」という条件を追加して対処した。

**★v1.13で設計改善：`url_mismatch`の判定基準自体を「ドメイン（netloc）の食い違い」に変更し、v1.12の`name_match`例外を撤回**。v1.12の対症療法には別の穴があった：全国に多数存在する同名企業（例：「株式会社田中製作所」の東京と大阪の無関係な別法人）が、CATV等の共用メールドメイン（+100点）と会社名一致（+100点）で200点に達した場合、`name_match=True`であるため`url_mismatch`足切りが無条件にスキップされ、Webサイトのドメインが全く異なる別法人であっても`possible_high`（電話・住所の追加一致次第では`exact_match`）に誤昇格してしまう（レビューで指摘）。この問題の根本原因は、v1.11〜v1.12の`url_mismatch`が「パスまで含めた正規化後URL」の食い違いを見ていたことにあった。**是正**：`url_mismatch`の判定基準を「Webサイトのドメイン（netloc）自体の食い違い」に変更した（詳細は`calculate_company_match()`のdocstring・`_extract_netloc()`参照）。これにより、同一ドメイン内のパス違い（トップページと下層ページ）はそもそも`url_mismatch`ではなく単なる`url_match=False`（加点なし、足切りもなし）として扱われるようになり、v1.12で導入した`not name_match`という迂回路が不要になったため撤回し、`determine_company_rank()`は**v1.11の原案どおり無条件の`if url_mismatch: return None`に戻した**。「同一会社の下層ページ違いによる見逃し」と「同名他社のドメイン違いによる誤結合」の両方を、判定関数側に特例を持たせることなく解消している。

| ランク | 必須条件 | スコア条件 | 実際に到達する組み合わせの例 |
|---|---|---|---|
| exact_match（自動リンク） | 会社名一致 AND (ドメイン一致 OR URL一致)、かつURLドメインが不一致でないこと | 220点以上 | 会社名+ドメイン+電話/住所いずれか1つ以上、または会社名+URL+電話/住所いずれか1つ以上（100+100+20=220） |
| possible_high | 会社名一致 AND (ドメイン一致 OR URL一致)、かつURLドメインが不一致でないこと | 200点（追加裏付けなし） | 会社名+ドメインのみ、または会社名+URLのみ（100+100=200点ちょうど） |
| possible_mid | (会社名一致 OR ドメイン一致 OR URL一致)、かつURLドメインが不一致でないこと | 140点以上 | 会社名+電話+住所（140点）、ドメイン+電話+住所（140点）、URL+電話+住所（140点） |
| possible_low（**★v1.11で是正**） | (会社名一致 OR ドメイン一致) AND **(電話一致 OR 住所一致)のいずれか1項目以上が必須**、かつURLドメインが不一致でないこと | 120点以上 | 会社名+電話または住所（120点）、ドメイン+電話または住所（120点）※社名変更後も同一メールドメイン・同一拠点（電話/住所）のケースを救済 |
| （候補外／`None`） | — | 120点未満、または単独一致（追加裏付けなし）、またはURLドメインが不一致 | 会社名のみ（100点）→`None`／**ドメインのみ（100点）→`None`（★v1.11新設）**／**URLのみ（100点）→`None`（★v1.11新設）**／電話のみ・住所のみ（各20点）→`None`／**双方のWebサイトのドメイン自体が明確に食い違う場合→`None`（★v1.11新設・★v1.13で判定基準をnetlocベースに是正、会社名一致でも例外なし）** |

```python
def determine_company_rank(score, name_match, domain_match, url_match, url_mismatch,
                            phone_match, address_match):
    """★v1.13で設計改善（★v1.11で新設・★v1.12で対症療法を施した
    url_mismatch足切りの根本是正）。
    ①URL不一致（url_mismatch=True）は最優先の足切りとする。v1.12までは
    name_matchが真の場合にこの足切りをスキップする例外を設けていたが、
    その例外には「ドメインが全く異なる同名の別法人が、共用メール
    ドメインだけでname_match=Trueになった場合、足切りをすり抜けて
    しまう」という穴があった（レビュー指摘）。v1.13でurl_mismatch自体の
    定義を「Webサイトのドメイン（netloc）自体の食い違い」に変更した
    （calculate_company_match()のdocstring参照）ことで、同一ドメイン内の
    パス違い（トップページ／下層ページ）はurl_mismatchにならなくなり、
    name_match例外を設けなくても「同一会社の下層ページ違いの誤足切り」を
    回避できるようになった。よって本関数はv1.11の原案どおり、
    name_matchを見ずに無条件でurl_mismatchを最優先の足切りとする。
    ②exact_match/possible_high は引き続き会社名一致を必須条件から外さない
    （ドメイン・URL・電話・住所が全部一致していても会社名が違えば
    自動リンクしない。OCR誤読の救済より、グループ会社間で連絡先を
    共有する別法人を誤って自動統合するリスクを避けることを優先した）。
    ③possible_mid は会社名・ドメイン・URLのいずれか1つが一致していれば
    候補になる（v1.3から変更なし）。
    ④possible_low は★v1.11で是正：単独一致（会社名のみ・ドメインのみ・
    URLのみ）は実データで無関係な別会社を誤って候補化する事象が確認された
    ため、(会社名一致 OR ドメイン一致) に加えて、電話・住所の
    いずれか1項目以上の追加裏付け一致を必須とし、閾値も120点以上に
    是正した。
    ⑤★v1.12で整理：has_backingはphone_match/address_matchのみを見る。
    url_match=Trueかつ(name_match or domain_match)=Trueの場合、スコアは
    必ず200点以上になりpossible_low到達前に上位ランクでreturnされるため、
    ここでurl_matchを見る意味はない（レビュー指摘、実害のないデッド
    コードだったため削除）。
    """
    if url_mismatch:
        return None  # ★v1.13でv1.11の原案どおりに復帰：name_match例外は撤回
    ...
    if (name_match or domain_match or url_match) and score >= 140:
        return 'possible_mid'
    has_backing = phone_match or address_match  # ★v1.12でurl_matchを除外（デッドコード整理）
    if (name_match or domain_match) and has_backing and score >= 120:
        return 'possible_low'
    return None  # 候補外（単独一致・120点未満・URLドメイン不一致を含む）
```
...
- **★v1.11で是正：`possible_low`は単独一致では到達できない**。v1.3〜v1.10は「会社名・ドメイン・URLのいずれか一致していれば、あとはスコア合計だけで判定する」形だったため、ドメイン単独一致（100点）が実データで候補化され、地域CATV共用ドメインや親会社共用ドメインによる無関係な別会社の誤結合を招いた。v1.11では`(会社名一致 OR ドメイン一致)`に加え、`(電話一致 OR 住所一致)`のいずれか1項目以上の追加裏付けを必須とし、閾値も140点未満から**120点以上**に是正した（**★v1.12で表記是正**：`has_backing`が実際に参照するのは`phone_match`／`address_match`のみで`url_match`は含まない。§6.5.2テーブルの旧「電話一致 OR 住所一致 OR URL一致」という記述はコードと食い違っていたため訂正した）。これにより「社名変更があったが同一メールドメインかつ同一拠点」という実務上あり得るケースは救済しつつ、裏付けのないドメイン共用だけの別会社は排除する
- **★v1.13で設計改善：`url_mismatch`はv1.11の原案どおり無条件の足切りに戻した**。詳細・具体例は本節冒頭を参照。判定基準を「ドメイン（netloc）自体の食い違い」に変更したことで、`name_match`の真偽に関わらず安全に無条件足切りできるようになった
- **参考（レビューで確認済み・変更不要）**：ドメインとURLの両方が一致し会社名が不一致というケース（200点）は`possible_high`（会社名一致が必須）には到達せず、`possible_mid`止まりとなる。これはv1.10から変更のない既存の挙動であり、「会社名がないドメイン系の一致だけで強いランクに乗せない」という今回の是正の方針とも整合するため、意図した挙動として維持する
...
### 6.5.3.1 重複候補レビュー画面の表示仕様（★v1.11で新設、★v1.12でソート優先度・exact_matchの扱いを補足、★v1.13でグループオブジェクトの構造を補足）

実データ運用で、全ランク一括のUnion-Findグルーピングが「野場電工同士の`possible_high`グループにノックス電子（`possible_low`相当）が巻き込まれ、1つの巨大カードとして誤表示される」という事故を招いたことを受け、Person重複マージ画面の設計思想（ランク別に分離して表示する）に合わせ、Company重複候補レビュー画面の表示仕様を以下の通り明文化する。

**① ランク別独立グルーピング（`_build_duplicate_groups()`）**

- `review_status='pending'`の`CompanyDuplicateCandidate`をグループ化してカード表示する`_build_duplicate_groups()`は、**全ランクを一括で連結するUnion-Findを禁止**する。
- グルーピングは`exact_match`／`possible_high`をまとめた区分、`possible_mid`、`possible_low`の**3区分それぞれで独立してUnion-Findを実行**し、異なる区分のCompanyを同一グループカードに束ねてはならない。`exact_match`と`possible_high`を同一区分にまとめるのは、両者が同じ必須条件（会社名＋(ドメインまたはURL)一致）を共有し、電話・住所の追加裏付けの有無だけでランクが分かれる関係にあるため（§6.5.2参照）。**★v1.12で補足**：`exact_match`は自動リンクされる（§6.5.2）ため、通常の運用フローでは`review_status='pending'`のまま`exact_match`候補が残ることは想定していない。この区分に含めているのは、自動リンク処理の失敗時の残骸や、レビュー担当者による手動降格など、将来のエッジケースで`exact_match`のpending候補が生じた場合に備えた保険的な定義であり、通常はこの区分に`possible_high`のみが実際に現れる想定でよい。
- 具体例：AとB（`possible_high`、200点）、AとC（`possible_low`、120点）という2件のペアが存在する場合、従来（全ランク一括Union-Find）ではA・B・Cが1つのグループにまとめられてしまうが、v1.11以降は「A・Bのグループ」（`exact_match`／`possible_high`区分）と「A・Cのグループ」（`possible_low`区分）に分離して、それぞれ別カードで表示する。
- グループカードの表示順は、**ランク順（`exact_match`／`possible_high` → `possible_mid` → `possible_low`）を優先し、同ランク内はスコア降順**とする。**★v1.12で明記**：優先度の数値体系は下記コード例の`RANK_PRIORITY`のとおり、数値が大きいほど優先度が高い（`exact_match`／`possible_high`＝3が最優先、`possible_low`＝1が最後）。

```python
# ★v1.13で追記：group（_union_find_groups()の戻り値の1要素）が想定する構造。
# 詳細な実装はコード君の判断に委ねるが、最低限以下の属性を持つ
# オブジェクト（dataclass等）またはそれと同等の辞書アクセスができる形とする。
#
# class DuplicateGroup:
#     candidates: list[CompanyDuplicateCandidate]  # このグループに属する候補ペア
#     companies: list[Company]                     # 候補ペアから展開した対象Company一覧
#     max_score: int                                # candidates内の最大score

# ★v1.12で新設：優先度は数値が大きいほど高い（exact_match/possible_high=3が最優先）。
# レビューで、_group_rank_priority()の戻り値の数値体系が未定義だったため、
# 実装者が「優先度1＝最優先」と誤解し符号(-)を逆に付けてexact_matchが
# 画面最下部に沈む表示順序バグを作り込むリスクが指摘された。
RANK_PRIORITY = {
    'exact_match': 3,
    'possible_high': 3,
    'possible_mid': 2,
...
def _group_rank_priority(group):
    """グループ内候補のうち最も優先度が高いランクの数値（RANK_PRIORITY）を返す。"""
    return max(RANK_PRIORITY.get(c.rank, 0) for c in group.candidates)


def _build_duplicate_groups(candidates):
    """★v1.11で全面是正。ランク区分ごとに独立してUnion-Findを実行し、
    異なる区分のCompanyを同一グループに束ねない。
    区分は (exact_match, possible_high) / possible_mid / possible_low の3つ。
    """
    RANK_GROUPS = (
        {'exact_match', 'possible_high'},
        {'possible_mid'},
        {'possible_low'},
    )
    groups = []
    for rank_set in RANK_GROUPS:
        rank_candidates = [c for c in candidates if c.rank in rank_set]
        groups.extend(_union_find_groups(rank_candidates))  # 既存のUnion-Find実装を区分内でのみ適用
    # ランク優先度（RANK_PRIORITY、数値が大きいほど優先）→ スコア降順でソートして返す
    return sorted(groups, key=lambda g: (-_group_rank_priority(g), -g.max_score))
```

**② 「HP（URL）」列の常設**

- 重複候補レビュー画面のテーブルは、スコア判定に使う5要素（社名・ドメイン・代表電話・HP・住所）を**すべてテーブル上で比較できるようにする**。既存の社名・ドメイン・電話・住所列に加え、**「HP」列（`Company.website`の生値を表示）を常設**する。
- グループ内の全社で正規化後URL（`normalize_website()`適用後）が一致している場合、ヘッダーに「URL一致」バッジを表示する。
- 先頭行（存続候補社、最古のCompanyまたはユーザーが選択した代表）とURLが異なる行は、背景色`#fffbeb`で差分ハイライトを適用する（Person重複マージ画面の値違いハイライトと同じ視覚言語）。
...
3. 既存Companyとスコア計算。`calculate_company_match()`（§6.5.1、★v1.3で`calculate_company_score()`から改称）をContact対Companyの呼び出し方（§6.5.1の呼び出し例参照）で使い、返り値の`match_result.score`／`.name_match`／`.domain_match`／`.url_match`／`.url_mismatch`／`.phone_match`／`.address_match`（**★v1.11で3項目追加**）を`determine_company_rank()`（§6.5.2）に渡してランクを求める
...
**★v1.1で確認：プレフィルタは変更不要**（★v1.12時点では一部不正確、下記参照）。§6.5.2の4段階ランクはいずれも「会社名一致」または「ドメイン一致」のどちらかを必須条件としており、電話・住所・URLだけが一致するケースは単独では候補にならない（§6.5.2の「候補外」参照）。したがってプレフィルタのOR条件に`phone`／`address`／`website`を加える必要はなく、`organization`／`domain`の2条件のままでよい。

**★v1.12で追記：既知の制約（性能対策としての意図的な割り切り）**。上記の確認はv1.1時点（URLがまだ弱い補助スコアだった頃）のものであり、v1.3でURLが会社名・ドメインと対等な主軸に格上げされた結果、実際には**`possible_mid`は`(会社名一致 OR ドメイン一致 OR URL一致)`のいずれか1つで到達できる**（§6.5.2）。このため、「社名不一致・ドメイン未設定（または汎用ドメイン）・URL一致＋電話・住所一致（計140点）」という`possible_mid`該当ケースは、本プレフィルタ（`organization`／`domain`の2条件のみ）では事前絞り込みの時点で0件になり、Pythonのスコア計算にすら到達せずサイレントに候補落ちする（レビューで指摘）。

`website`はCompany件数増加時にフルスキャンを招くリスクがあるため索引を張りにくく、プレフィルタのOR句に`website`を追加することは性能面のトレードオフが大きい。**たんたんの判断により、この既知の制約は不具合として修正せず、意図的な性能対策上の割り切りとして維持する**。将来的にCompany件数が増え、この取りこぼしが運用上無視できなくなった場合は、`website`（正規化後の値）にも`db_index=True`を張った上でプレフィルタに追加することを次フェーズで検討する。
...
**★v1.15で新たに確定した事項**：実装着手前レビュー（2名）で「v1.14は完全LGTM・完全FIX（凍結）」の判定を受けた。申し送り1点（`_normalize_url_string()`単体呼び出し時の防御）のみを反映し最終確定。業務ルール・スコア・ランク判定・画面仕様の変更はなし。§9.2の実装指示書向け申し送り事項（関数配置3点・ソート基準・プレフィルタの既知の仕様）が揃っていることをレビューで確認済み。詳細は§6.5.1を参照。

**★v1.14で新たに確定した事項**：実装着手前レビュー（2名）で「v1.13はLGTM（合格）、仕様書はFIX（凍結）可」の判定を受けた。指摘は軽微な1点（`normalize_website()`と`_extract_netloc()`のnetloc抽出ロジックの重複）のみで、業務ルール・スコア・ランク判定・画面仕様の変更はなし。`normalize_website()`内部で`_extract_netloc()`を呼ぶ形に整理した。あわせて、実装指示書作成時に明記すべきモジュール配置（`_check_address_match`・`_extract_netloc`は`companies/services.py`、`normalize_phone_value`は`contacts/services/normalization.py`）をレビューでの申し送りとして記録した。詳細は§6.5.1を参照。

**★v1.13で新たに確定した事項**：実装着手前レビュー（2名）で判明した3点を是正。①【最重要・即死級バグ】`calculate_company_match()`の住所前方一致判定が「先頭5文字スライスの単純比較」になっており、同一市区町村内の無関係な別住所同士まで一致扱いにしてしまう欠陥を修正。既存の`_check_address_match()`（`a.startswith(b) or b.startswith(a)`方式、companies/services.py、実装済み）を呼ぶ形に是正。②電話・住所の正規化を、新規インライン実装から既存関数（`normalize_phone_value()`・`_check_address_match()`）の呼び出しに統一（v1.12は実コードとの食い違いにより新規実装が既存実装より機能後退していた）。③`url_mismatch`の判定基準を「正規化後URL（ドメイン＋パス）の食い違い」から「Webサイトのドメイン（netloc）自体の食い違い」に変更し、v1.12で導入した`determine_company_rank()`側の`not name_match`例外（同名他社が別ドメインを持つ場合に誤って足切りをすり抜ける穴があった）を撤回、v1.11の原案どおり無条件の足切りに復帰。あわせて§6.5.2の`possible_low`必須条件テーブルの表記ズレ（コードは`has_backing = phone_match or address_match`のみだが、テーブルに`URL一致`が残っていた）を修正し、`_build_duplicate_groups()`のグループオブジェクトの構造を補足。詳細は§6.5.1、§6.5.2、§6.5.3.1を参照。

**★v1.12で新たに確定した事項**：実装着手前レビュー（2名）で判明した5点を是正。①`determine_company_rank()`の`url_mismatch`足切りを、`name_match`が真の場合はスキップするよう修正（会社名完全一致の重複が無条件に見逃される事故を防止、最重要）。②`calculate_company_match()`の電話・住所比較を、スコア表が定める正規化（数字抽出・空白除去＋前方一致）に修正（生文字列比較のままだった欠陥）。③`normalize_website()`に`index.html`等のトップページファイル名の除外ガードを追加（過剰なURL不一致検出を防止）。④§6.5.4のプレフィルタが`possible_mid`のURL単独裏付けケースを拾えない点を、意図的な性能対策上の割り切りとして明記。⑤`_build_duplicate_groups()`のソート優先度（`RANK_PRIORITY`）を明記し、`determine_company_rank()`の`has_backing`から実害のないデッドコード（`url_match`）を除去。詳細は§6.5.1、§6.5.2、§6.5.3.1、§6.5.4を参照。

**★v1.11で新たに確定した事項**：実データ運用で確認された2つの誤結合事象（地域CATV等の共用ドメイン、親会社・グループ企業間の共用独自ドメイン）を受け、Company重複判定を全面是正。①ドメイン単独一致・URL単独一致（各100点）を候補化廃止・足切り（`None`）に変更。②`possible_low`の閾値を140点未満から120点以上に変更し、`(会社名一致 OR ドメイン一致) AND (電話一致 OR 住所一致 OR URL一致)`を必須条件化。③双方の`website`が明確に食い違う場合の不一致足切り（`url_mismatch`）を新設し、`determine_company_rank()`の最優先チェックとした。④会社名の一致判定を`normalize_organization()`経由に明確化。⑤重複候補レビュー画面のランク別独立Union-Find（`_build_duplicate_groups()`）とHP（URL）列常設をUI仕様として明文化（§6.5.3.1、新設）。詳細は§6.5.1、§6.5.2、§6.5.3.1を参照。
...
| バージョン | 日付 | 内容 |
|---|---|---|
| v1.15 | 2026-09-15 | 実装着手前レビュー（2名）で「v1.14は完全LGTM・完全FIX（凍結）」の判定。申し送り1点（`_normalize_url_string()`が将来単体で呼び出された場合に備えた防御`if not url: return ''`の追加）のみを反映し最終確定。業務ルール・スコア・ランク判定・画面仕様の変更なし。§9.2の実装指示書向け申し送り（関数配置3点・ソート基準・プレフィルタの既知の仕様）が揃っていることをレビューで確認済み。影響箇所：§6.5.1 |
| v1.14 | 2026-09-15 | 実装着手前レビュー（2名）で「v1.13はLGTM、FIX（凍結）可」の判定。指摘は軽微な1点（`normalize_website()`と`_extract_netloc()`のnetloc抽出ロジックの重複）のみで、業務ルール・スコア・ランク判定・画面仕様の変更なし。`normalize_website()`内部で`_extract_netloc()`を呼ぶ形に整理し、コードの重複を解消。実装指示書作成時に明記すべきモジュール配置（`_check_address_match`・`_extract_netloc`は`companies/services.py`、`normalize_phone_value`は`contacts/services/normalization.py`）をレビューでの申し送りとして記録。影響箇所：§6.5.1、§9.1 |
| v1.13 | 2026-09-15 | 実装着手前レビュー（2名）で判明した3点を反映。**①【最重要・即死級バグ】**`calculate_company_match()`の住所前方一致判定が`normalized_address_a[:5] == normalized_address_b[:5]`（先頭5文字スライスの単純比較）になっており、日本の住所では先頭5文字が「都道府県名＋市区町村名」に相当するため、同一市区町村内の無関係な別住所同士（例：「愛知県豊田市西町」と「愛知県豊田市日之出町」）まで住所一致（+20点）を獲得してしまい、CATV等の共用ドメイン（100点）と組み合わさって`possible_low`（120点）に大量に誤到達する欠陥を修正。実コードベース（`companies/services.py`）を確認した結果、正しい前方一致（`a.startswith(b) or b.startswith(a)`、5文字以上）は既に`_check_address_match()`として実装済みであることが判明したため、新規コードを削除し既存関数を呼ぶ形に是正。あわせて電話の正規化も、v1.12の新規インライン実装（全角数字・漢数字の変換漏れ）から既存の`normalize_phone_value()`の呼び出しに統一。**②【設計改善】**`url_mismatch`の判定基準を「正規化後URL（ドメイン＋パス）の食い違い」から「Webサイトのドメイン（netloc）自体の食い違い」に変更。v1.12で導入した`determine_company_rank()`側の`not name_match`例外には、「ドメインが全く異なる同名の別法人が、共用メールドメインだけで`name_match=True`になった場合に足切りをすり抜けてしまう」という別の穴があったため、根本原因（`url_mismatch`がパスまで見ていたこと）を是正し、この例外を撤回してv1.11の原案どおり無条件の`if url_mismatch: return None`に復帰。**③**§6.5.2の`possible_low`必須条件テーブルに残っていた`URL一致`の表記（v1.12でコード側の`has_backing`からは既に除外済み）を削除し、テーブルとコードの食い違いを解消。あわせて`_build_duplicate_groups()`のグループオブジェクトの想定構造を補足。影響箇所：§6.5.1、§6.5.2、§6.5.3.1、§9.1 |
| v1.12 | 2026-09-15 | 実装着手前レビュー（2名）で判明した5点を反映。**①【最重要】**`determine_company_rank()`の`url_mismatch`足切りが`name_match`を見ずに無条件・最優先で発動し、会社名・ドメイン完全一致の明白な重複（220点超）までURLパスの些末な違い（トップページと下層ページ等）だけで候補外に握りつぶしていた欠陥を修正。`url_mismatch`足切りは`name_match`が偽の場合のみ適用するよう是正（§6.5.2）。②`calculate_company_match()`の電話・住所比較が生の文字列完全一致のままで、スコア表（§6.5.1）が定めた正規化（電話：数字抽出、住所：空白除去＋前方一致）と食い違っていた欠陥を修正。ハイフン有無等の表記ゆれで`possible_low`の追加裏付けが不成立になる取りこぼしを解消。③`normalize_website()`が`index.html`／`index.htm`／`index.php`等のトップページファイル名の有無だけで別URL扱いしてしまう過剰検出を、セグメント抽出時の除外ガードで是正。④§6.5.4のプレフィルタ（`organization`／`domain`の2条件のみ）が`possible_mid`のURL単独裏付けケースを拾えない点について、v1.1確認の記述を訂正し、意図的な性能対策上の割り切りとして明記。⑤`_build_duplicate_groups()`のソート処理が参照する`_group_rank_priority()`の数値体系（`RANK_PRIORITY`）を明記し、表示順序の逆転バグを予防。`determine_company_rank()`の`has_backing`から到達不能だった`url_match`参照を除去し、`phone_match or address_match`に整理。影響箇所：§6.5.1、§6.5.2、§6.5.3.1、§6.5.4、§9.1 |
| v1.11 | 2026-09-15 | 実データ（名刺インポート・会社リンク）運用で確認された2つの誤結合事象（①`aitai.ne.jp`等の地域CATV共用ドメインによる無関係企業の誤結合、②「ノックス電子」が親会社「野場電工」の共用独自ドメインを使っていたことによる巻き込み）を受け、Company重複判定を全面是正。§6.5.1：会社名一致判定を`normalize_organization()`経由に明確化。ドメイン単独一致・URL単独一致（各100点）の候補化を廃止し足切り（`None`）とする新ルール、双方の`website`が明確に食い違う場合の不一致足切り（`url_mismatch`）を新設。§6.5.2：`determine_company_rank()`にURL不一致の最優先足切りを追加。`possible_low`の必須条件を「(会社名一致 OR ドメイン一致) AND (電話一致 OR 住所一致 OR URL一致)」に是正し、閾値を140点未満から120点以上に変更。§6.5.3.1（新設）：重複候補レビュー画面の`_build_duplicate_groups()`をランク別（`exact_match`／`possible_high`、`possible_mid`、`possible_low`）独立Union-Findに是正し、全ランク一括連結を禁止。HP（URL）列の常設・URL一致バッジ・差分ハイライトを明文化。影響箇所：§6.5.1、§6.5.2、§6.5.3.1、§6.5.4、§9.1 |

...
バージョン v1.6.1
FreeGroup2 グループウェア
2026年5月作成 / OCR・OpenCV 章を v1.6.0 系 3 本に分離し本編仕様書として再編（v1.6.1 で電話系巻き戻し・organization.website 追加・ContactSns 別テーブル化を反映）
...
v1.6.1 改訂：SNS は ContactSns 別テーブル化されたため、比較表示の単位を「個別フィールド」から「ContactSns レコード」に変更する。両 Person の ContactSns レコードを sns_type 別に並べ、両側で同じ sns_type を持つもの・片側のみ持つものを視覚的に比較できるよう表示する。表示制御の具体的な UI 設計はコード君A の実装時判断（既存の SNS フィールド比較表示の挙動を踏襲）。
...
### 11.6.7 ContactSns 編集 UI（InlineFormSet）（v1.6.1 新設）
...
- 具体的な UI 設計・テンプレート構造はコード君A の実装時判断

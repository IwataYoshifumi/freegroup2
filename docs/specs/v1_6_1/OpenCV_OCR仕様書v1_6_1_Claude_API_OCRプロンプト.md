...
- `website`：名刺記載のウェブサイト URL。プロトコル付き完全 URL（例：`https://example.com/`）。会社の公式ウェブサイトとして名刺に記載されている URL を格納する。SNS アカウントやブログ等は `sns` ブロックに格納し、ここには含めない
...
- `email`：メールアドレス（1件のみ）
- 電話・FAX 各フィールド：**すべて E.164 形式（例：`+819012345678`）に変換して格納すること**。名刺に複数記載がある場合は、本人の主たる 1 つを採用する。残りの番号は raw_json 内のみに保持され、Contact には反映されない（1 名刺 = 1 Contact、主たる 1 つを保存する設計原則による）
  - `mobile_phone`：本人の携帯電話番号（単一値）
  - `personal_phone`：本人の個人直通電話番号（単一値）
  - `personal_fax`：本人の FAX 番号（単一値）
  - `org_phone`：会社・組織の代表電話番号、部署直通電話番号（単一値）
  - `org_fax`：会社・組織の代表 FAX 番号、部署 FAX 番号（単一値）
- `sns`：名刺に記載されている SNS アカウント。`type`（小文字統一、以下の 8 種から選択）と `id`（URL またはユーザー ID）のオブジェクトの配列として格納
  - 受理する type：`twitter` / `linkedin` / `facebook` / `instagram` / `github` / `blog` / `youtube` / `line`
  - 表記揺れ（`LinkedIn` / `X` / `WeChat` 等）は json_parser 側で正規化されるが、可能な限り OCR 出力時点で上記 8 種・小文字統一で出力すること
  - 会社の公式ウェブサイト URL は `sns` に含めず、`organization.website` に格納する
  - 上記 8 種に該当しない SNS（WeChat / TikTok / Threads 等）は出力しなくてよい（json_parser でサイレント無視されるため）
...
配列フィールド（sns のみ）の confidence は配列全体に1つ。要素ごとに信頼度が異なる場合は最も低い要素の confidence を全体に採用。空配列（`[]`）の confidence は `"high"`（「該当記載なしと確定した」が正解）。

電話系 5 フィールド（mobile_phone / personal_phone / personal_fax / org_phone / org_fax）は v1.6.1 で単一値（string/null）となったため、本ルールの対象外。null の confidence は `"high"`（「該当記載なしと確定した」が正解）。

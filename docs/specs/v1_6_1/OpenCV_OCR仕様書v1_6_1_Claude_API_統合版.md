# OpenCV・OCR仕様書 v1.6.1（Claude.API）統合版

バージョン：v1.6.1（Claude.API・統合版）
作成日：2026-05-20（v1.6.1 改訂：2026-05-23）
位置付け：FreeGroup2 次フェーズ OCR 改善の本体仕様書。

本書は FreeGroup2 次フェーズ OCR 改善の本体仕様書。OCR 系仕様書は3本構成。

1. OpenCV・OCR仕様書v1.6.1（Claude.API）統合版.md（本書）
2. OpenCV・OCR仕様書v1.6.1（Claude.API）OCRプロンプト.md（OCR バックエンド：Claude Sonnet 4.6、Tool Use 構造化 JSON 取得）
3. OpenCV・OCR仕様書v1.6.1（Claude.API）JSON構造・コンタクトフィールド対応表.md（OCR 出力 JSON 構造と Contact フィールドの対応）

本書は重複検出スコアリング詳細・マージ機構・View・URL・認証等の OCR と無関係な領域も含む統合本体。OCR 系の中核仕様は2と3を参照すること。3本のバージョンは v1.6.1 で揃える。
...
```
{
  ...
  "organization": {
    ...
    "website":                    {"value": null, "confidence": "high"}   // string/null。名刺記載の会社ウェブサイトURL（既存Contact.websiteに同名ストレート流し・v1.6.1新設）
  },
  "address": {
  ...
  "contact": {
    ...
    "personal_phone": {"value": null, "confidence": "high"},  // string/null。本人個人直通・単一値・E.164（v1.6.1で配列→単一値）
    "personal_fax":   {"value": null, "confidence": "high"},  // string/null。本人FAX・単一値・E.164（v1.6.1で配列→単一値）
    "org_phone":      {"value": null, "confidence": "high"},  // string/null。会社代表・部署電話・単一値・E.164（v1.6.1で配列→単一値）
    "org_fax":        {"value": null, "confidence": "high"},  // string/null。会社・部署FAX・単一値・E.164（v1.6.1で配列→単一値）
    "sns":            {"value": [],   "confidence": "high"}   // array of object {type, id}。ContactSns別テーブルに分配（v1.6.1）
  },
...
- **電話系5フィールド**：mobile_phone（本人携帯・単一値）/ personal_phone（本人個人直通・単一値）/ personal_fax（本人FAX・単一値）/ org_phone（会社代表・部署電話・単一値）/ org_fax（会社FAX・単一値）。すべて E.164 形式・単一値（string/null）。**v1.6.1 で personal_phone / personal_fax / org_phone / org_fax を v1.6.0 の配列（JSONField）から単一値（CharField）に巻き戻した**（前任サポート担当 Claude の見落としによる仕様逸脱の修正、§3.1 参照）。複数記載時は本人の主たる 1 つを採用し残りは raw_json に残す（1 名刺 = 1 Contact 原則）。**org_mobile_phone は作らない**（携帯は本人限定。組織携帯は個人/組織の区別を曖昧にし重複検出誤判定リスク）。代表電話と部署電話は org_phone に統合（区別する業務上の必要が現状ない）
...
- 配列フィールド（sns のみ）の confidence は配列全体に1つ。要素ごとに信頼度が異なる場合は最も低い要素の confidence を全体に採用。空配列の confidence は high（「該当記載なしと確定した」が正解）。電話系5フィールドは v1.6.1 で単一値（string/null）となったため対象外（null の confidence は high）
...
OCRプロンプト本文の正本は `OpenCV・OCR仕様書v1.6.1（Claude.API）OCRプロンプト.md`（添付2）。本書はその採用宣言と要点を記す。プロンプト本文と本書が食い違った場合は添付2を正とする。採用後 `cards/prompts/extract_combined.txt` に反映。
...
| phone | personal_phone | CharField(50) | CharField(50) | リネームのみ |
| fax | personal_fax | CharField(50) | CharField(50) | リネームのみ |

リネーム 4 件はいずれも型変更を伴わない（CharField のまま）。マイグレーションは RenameField のみ。

【v1.6.1 改訂】 v1.6.0 では phone / fax を JSONField(default=list) に型変更する記述だったが、これは前任サポート担当 Claude の見落としによる仕様逸脱だったため、v1.6.1 で CharField(50) に巻き戻した。v1.1.0 で確定した「1 名刺 = 1 Contact、主たる 1 つを保存、他は raw_json に残る」原則に整合させる。OCR 出力 JSON も単一値（string/null）に変更（OCR プロンプト仕様書 §6 参照）。
...
| 18 | org_phone | CharField(50) |
| 19 | org_fax | CharField(50) |

personal_phone / personal_fax は §3.1 のリネームで扱うため本一覧に含めない（v1.6.1 で型変更なし・CharField(50) のまま）。
...
### 3.3.2 website のストレート流し込み（v1.6.1 新設）

OCR 出力 JSON の organization.website は、既存 Contact.website（v1.4.4 から存在、CharField(500)）にそのまま流し込む。橋渡し処理は不要（OCR キー名と Contact フィールド名が同一）。Contact フィールドの追加は不要（既存流用）。マイグレーションは不要。

| OCRキー | Contactフィールド | 処理種別 |
|---|---|---|
| organization.website | website | ストレート流し（橋渡し不要） |
...
**入れる（新規追加分）**：name_order / other_name_parts / display_name / phonetic_name / alias_name / organization（旧company・継続）/ legal_entity_type / legal_entity_type_position / region / city / rest_of_address / country / postal_code（継続）/ mobile_phone（旧mobile・継続）/ personal_phone（旧phone・継続、**CharField(50) 単一値**）/ personal_fax（旧fax・継続、**CharField(50) 単一値**）/ org_phone（新規、**CharField(50)**）/ org_fax（新規、**CharField(50)**）/ website（既存・v1.6.1 で OCR organization.website 流入経路新設に伴い明示）。full_name は既存 UPDATABLE 継続

**v1.6.1 改訂（SNS）：** 個別 SNS フィールド 5 件（twitter / instagram / github / linkedin / facebook）は ContactSns 別テーブル化に伴い廃止し、UPDATABLE_FIELDS からも削除する。ContactSns の編集は InlineFormSet で別途扱う（本編仕様書 §11.6 参照）。
...
## 3.7 ContactSns 別テーブル（v1.6.1 新設）
...
### 3.7.2 SnsType choices（8 種、小文字統一）
...
- type のバリエーション拡張に DB 変更不要
- 1 名刺で複数 YouTube チャンネル、Twitter 個人/会社両方、等のケースに対応可能
- 個別 SNS フィールド方式の不整合（「LinkedIn だけ CharField(500)、他は CharField(255)」のような微妙な長さ違い）を解消
- 「LINE だけ抜き出したい」等の検索ニーズに JOIN で対応可能（`Contact.objects.filter(sns_accounts__sns_type="line")`）
- ユーザーが直接編集する JSONField を増やさずに済む

### 3.7.4 UniqueConstraint の意味

同一 Contact に対して、同じ sns_type かつ同じ sns_id の組み合わせを重複登録しない。twitter の `@example` と twitter の `@another` は両立する（同一 sns_type で複数チャンネル可）。

### 3.7.5 json_parser でのレコード生成ロジック

OCR が出力する type は表記揺れがあり得る。json_parser で正規化してから ContactSns レコード作成する。
...
for entry in raw_json["contact"]["sns"]["value"]:
    ...
    if normalized_type not in SnsType.values:
        continue  # choices 外はサイレント無視
    if not raw_id:
        continue
    ContactSns.objects.get_or_create(
        contact=contact, sns_type=normalized_type, sns_id=raw_id,
    )
```

choices 外の type は v1.6.1 ではサイレント無視。other 受け皿の必要性は v1.7.0 以降で運用データを見て判断。

### 3.7.6 既存個別 SNS フィールドの廃止

Contact から twitter / instagram / github（各 CharField）/ linkedin / facebook（各 CharField(500)）の 5 フィールドを削除する。開発 DB は削除可（マイグレーション時のデータ移行は不要）。
...
旧 v1.3.0 は fields/fields_array 2階層フラット構造。v1.6.0 → v1.6.1 で8ブロック構造に作り直し、フィールド大幅増。読み取りロジックとフィールドマッピングは全面作り直し。純関数性・防御実装・orientation 補正は維持。電話系 5 フィールド（mobile_phone / personal_phone / personal_fax / org_phone / org_fax）は v1.6.1 で単一値（CharField(50)）として Contact に格納する（v1.6.0 で一時的に JSONField 配列としていた 4 件を巻き戻し、§3.1 参照）。sns は v1.6.1 で ContactSns 別テーブルにレコード分配する（§3.7 参照）。旧「先頭 1 件のみ採用」は廃止。
...
| job（4） | 全て既存・同名 |
| organization（8） | org_name_full→organization（名前変換）、他7件同名（org_core_name/legal_entity_type/legal_entity_type_code/legal_entity_type_position/org_domain_name 新設、branch 既存、website は v1.6.1 新設・既存 Contact.website に同名ストレート流し） |
| address（6） | full_address→address（名前変換）、他5件同名（postal_code 既存、country/region/city/rest_of_address 新設） |
| contact（7） | email/mobile_phone/personal_phone/personal_fax は既存（リネーム後名・v1.6.1 で全て CharField(50) 単一値）、org_phone/org_fax 新設（CharField(50)）、sns は v1.6.1 で ContactSns 別テーブルへ分配（type 正規化付き、§3.7 参照） |
...
| D：導出 | 他フィールドから生成 | full_address（4要素から）/org_core_name（org_name_full から）/org_domain_name（email から）/legal_entity_type_code（legal_entity_type から）/full_name（original_script からコピー） |
...
**website について（v1.6.1）：** website は v1.6.1 で OCR からの流入経路が新設されたが、正規化対象外のまま継続する。URL の正規化（プロトコル統一・トレイリングスラッシュ等）は v1.7.0 以降の検討事項とする。
...
v1.6.1 改訂：personal_phone は v1.6.0 で一時的に JSONField 化されていたが、v1.6.1 で CharField(50) に巻き戻されたため、DUPLICATE_CHECK_FIELDS の構成・スコア・判定ロジックに影響なし。
...
| v1.6.0（Claude.API）統合版 | 2026-05-20 | OCR は full_name を出力しない設計に変更（json_parser が original_script をコピー）。original_script を Contact フィールドから外し raw_json 内のみに（新規追加 20→19件）。phonetic_name は記載なしでも推測 low（lang=ja 限定・カタカナ表記）。salutation_name 手動入力時必須化。confidence mid 統一を本フェーズスコープ内で確定。name ブロック内整合性チェック4種類を normalization 基盤に追加（§2.4.1）。ファイル名・バージョンを OCR 系3本で v1.6.0 に統一 |
| v1.6.1（Claude.API）統合版 | 2026-05-23 | 電話系 4 フィールド（personal_phone / personal_fax / org_phone / org_fax）を JSONField → CharField(50) に巻き戻し（v1.6.0 で JSONField 化したのは前任サポート担当 Claude の見落としによる仕様逸脱だったため修正）。organization.website を新設（既存 Contact.website に同名ストレート流し込み）。Contact 個別 SNS フィールド 5 件を廃止し ContactSns 別テーブル化（sns_type choices 8 種：twitter / linkedin / facebook / instagram / github / blog / youtube / line、json_parser で type 正規化）。ファイル名・バージョンを OCR 系3本で v1.6.1 に統一 |

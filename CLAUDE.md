...
- **バージョン付きファイル運用（厳守）**：仕様書・URL一覧表等のドキュメントは、**内容が変わったときは必ずファイル名のバージョン番号を上げる**（旧版は残置）。プロジェクトに取り込むときにどれが最新か分からなくなるのを防ぐため。例：`..._rev17.md` → `..._rev17_1.md` → `..._rev18.md`。ファイル名の小数点はアンダースコア表記（`rev17_1`、Linux/Windows どちらでも安全）。軽微な整合修正でも内容が変わるなら新ファイル化する。**逆に、内容が変わらないなら原則バージョンを変えない**（同一内容で版番号だけ増やさない）。**内容不変でバージョンを変えたい場合は、必ずたんたんの了承を得てから行う**
- **生成ファイルは毎回チャットに出力する（厳守）**：ドキュメントを新規作成・改訂したら、毎回その成果物ファイルをチャット上に出力（送付）する。たんたんから指示がなくても自動で行う

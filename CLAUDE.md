...
- **バージョン付きファイル運用（厳守）**：仕様書・URL一覧表等のドキュメントは、**少しでも内容を変更したらファイル名のバージョン番号を必ず上げる**（旧版は残置）。プロジェクトに取り込むときにどれが最新か分からなくなるのを防ぐため。例：`..._rev17.md` → `..._rev17_1.md` → `..._rev18.md`。ファイル名の小数点はアンダースコア表記（`rev17_1`、Linux/Windows どちらでも安全）。軽微な整合修正でも新ファイル化する
